...
*/
const MODULE_ID = 'aca-test-cases-datasource-mongo-test-workers-save-one';
...
const saveOne = async (datasource, context, params) => {
  ...
  let filter;
  try {
    const INSTANCE = ramda.path(['worker'], params);
    if (
      lodash.isEmpty(INSTANCE)
    ) {
      const MESSAGE = `Missing required params.worker parameter!`;
