...
*/
const MODULE_ID = 'aca-test-cases-datasource-provider-datasource-factory';
...
const { AcaTestCasesDatasourceMongo } = require('../datasource-mongo');
...
const _createMongoDatasource = async (configuration) => {
  try {
    ...
    const RET_VAL = new AcaTestCasesDatasourceMongo(configuration);
...
const createAcaTestCasesDatasource = async (configuration) => {
  try {
    ...
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error(createAcaTestCasesDatasource.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

const createAcaTestCasesDatasources = async (configurations) => {
  try {
    if (
    ...
    const PROMISES = configurations.map((configuration) => {
      return createAcaTestCasesDatasource(configuration);
    });
    const RET_VAL = await Promise.all(PROMISES);
    return RET_VAL;
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error(createAcaTestCasesDatasources.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

module.exports = {
  createAcaTestCasesDatasource,
  createAcaTestCasesDatasources,
}
