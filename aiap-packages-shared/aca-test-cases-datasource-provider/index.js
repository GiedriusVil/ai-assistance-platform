...
*/
const MODULE_ID = 'aca-test-cases-datasource-provider';
...
const { createAcaTestCasesDatasources } = require('./lib/datasource-factory');
...
const initManyByRootConfiguration = async (path = DEFAULT_CONFIG_PATH) => {
  try {
    ...
    if (
      ...
      const DATASOURCES = await createAcaTestCasesDatasources(CONFIG_DATASOURCES);
...
const initManyByTenant = async (params) => {
  try {
    ...
    const CONFIG_DATASOURCES_CONVERSATIONS = lodash.filter(CONFIG_DATASOURCES_ALL, { type: 'testCases' });
    if (
      !lodash.isEmpty(CONFIG_DATASOURCES_CONVERSATIONS)
    ) {
      const DATASOURCES = await createAcaTestCasesDatasources(CONFIG_DATASOURCES_CONVERSATIONS);
...
const getAcaTestCasesDatasourceByContext = (context) => {
  try {
    ...
    const RET_VAL = getAcaTestCasesDatasourceByTenant(TENANT);
    return RET_VAL;
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error(getAcaTestCasesDatasourceByContext.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

const getAcaTestCasesDatasourceByTenant = (tenant) => {
  try {
    ...
    if (
      ...
      const DATASOURCE_INDEX = lodash.findIndex(DATASOURCES_CONFIG, { type: 'testCases' })
      if (
        DATASOURCE_INDEX >= 0
      ) {
        ...
        retVal = getAcaTestCasesDatasource(DATASOURCE_ID, DATASOURCE_HASH);
      }
    }
    return retVal;
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error(getAcaTestCasesDatasourceByTenant.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

const getAcaTestCasesDatasource = (id = 'default', hash = undefined) => {
  try {
    let retVal;
    if (
      !lodash.isEmpty(id) &&
    ...
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error(getAcaTestCasesDatasource.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

const getAcaTestCasesDatasources = (context) => {
...
module.exports = {
  ...
  initManyByTenant,
  getAcaTestCasesDatasources,
  getAcaTestCasesDatasource,
  getAcaTestCasesDatasourceByContext,
  getAcaTestCasesDatasourceByTenant,
}
