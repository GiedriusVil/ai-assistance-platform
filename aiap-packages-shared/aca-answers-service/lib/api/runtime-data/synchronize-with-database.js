...
*/
const MODULE_ID = 'aca-ai-services-runtime-data-service-synchronize-with-database';
...
const { synchronizeWithDatabaseAnswersStoresByTenant } = require('./synchronize-with-database-answers-stores-by-tenant');
...
const _synchronizeWithDatabaseByTenant = async (context, params) => {
  ...
  try {
    ...
    const ANSWERS_STORES = await synchronizeWithDatabaseAnswersStoresByTenant(CONTEXT, params);
    const RET_VAL = {
      tenantId: params?.tenantId,
      answersStores: ANSWERS_STORES,
    };
    return RET_VAL;
  } catch (error) {
    ...
    if (
      ...
      if (
        ...
        for (let result of PROMISES_RESULTS) {
          logger.info(`TenantCustomizer AnswersStores synchronized [TenantId: ${result?.tenantId}, QTY.: ${result?.answersStores?.length}]`);
        }
      }
    }
  } catch (error) {
