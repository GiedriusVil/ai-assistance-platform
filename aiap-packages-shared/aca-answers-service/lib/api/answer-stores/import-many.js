...
*/
const MODULE_ID = 'aca-answer-stores-service-answer-stores-import-many';
...
const importMany = async (context, params) => {
  try {
    ...
    if (
      ...
      const MESSAGE = 'Missing answers stores in file!';
    ...
    if (
      ...
      const MESSAGE = `Answers stores are not compatible for import! File must contain 'id'!`;
      throwAcaError(MODULE_ID, ACA_ERROR_TYPE.VALIDATION_ERROR, MESSAGE);
    }
    const PROMISES = [];
    for (let answerStore of RECORDS_FROM_FILE) {
      PROMISES.push(saveOne(context, { answerStore }));
    }
