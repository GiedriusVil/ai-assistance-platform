...
*/
const MODULE_ID = 'aca-auditor-express-routes-controller-lambda-modules-find-many-by-query';
...
const { lambdaModulesAuditorService } = require('@ibm-aca/aca-auditor-service');
...
const findManyByQuery = async (request, response) => {
  ...
  try {
    ...
    if (lodash.isEmpty(ERRORS)) {
      result = await lambdaModulesAuditorService.findManyByQuery(CONTEXT, PARAMS);
    }
  } catch (error) {
