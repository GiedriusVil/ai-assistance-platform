...
*/
const MODULE_ID = 'aca-answers-datasource-provider-datasource-factory';
...
const { AcaAnswersDatasourceMongo } = require('../datasource-mongo');
...
const _createMongoDatasource = async (configuration) => {
  try {
    ...
    const RET_VAL = new AcaAnswersDatasourceMongo(configuration);
...
const createAcaAnswersDatasource = async (configuration) => {
  try {
    ...
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error(createAcaAnswersDatasource.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

const createAcaAnswersDatasources = async (configurations) => {
  try {
    if (
    ...
    const PROMISES = configurations.map((configuration) => {
      return createAcaAnswersDatasource(configuration);
    });
    const RET_VAL = await Promise.all(PROMISES);
    return RET_VAL;
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error(createAcaAnswersDatasources.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

module.exports = {
  createAcaAnswersDatasource,
  createAcaAnswersDatasources,
}
