...
*/
const MODULE_ID = 'answers-datasource-mongo-answer-stores-find-many-by-query';
...
const _retrieveLatestAnswerStoreRelease = async (datasource, context, params, ANSWER_STORES) => {
  ...
  try {
    ...
    ANSWER_STORES.forEach((store) => {
      let answerStoreLatestRelease = RESULT.find((item) => {
        let condition = false;
      ...
      if (!lodash.isEmpty(answerStoreLatestRelease)) {
        store.latestReleaseDeployedTime = ramda.path(['items', 0, 'deployedT'], answerStoreLatestRelease);
      }
    });
