...
*/
const MODULE_ID = `aca-catalog-rules-engine-provider-json-rule-engine-transform-rule-to-native-format`;
...
const transformRuleToNativeFormat = (rule) => {
  let name;
  let conditions;
  try {
    name = rule?.name;
    ...
    logger.error(`${transformRuleToNativeFormat.name}`, { ACA_ERROR });
    throw ACA_ERROR;
  }
};

module.exports = {
  transformRuleToNativeFormat,
}
