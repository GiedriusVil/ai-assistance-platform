...
*/
const MODULE_ID = 'aca-validation-engagements-express-routes-controller-validation-engagements-find-one-by-rule-id';
...
const findOneByRuleId = async (request, response) => {
  const ERRORS = [];
  ...
  try {
    ...
    result = await validationEngagementsService.findOneByRuleId(CONTEXT, PARAMS);
  } catch (error) {
  ...
  if (lodash.isEmpty(ERRORS)) {
    ...
    logger.error(findOneByRuleId.name, { ERRORS });
