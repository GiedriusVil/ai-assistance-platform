...
*/
const MODULE_ID = 'aca-validation-engagements-express-routes-controller-validation-engagements-get-paths-by-id';
...
const getPathsById = async (request, response) => {
  const ERRORS = [];
  ...
  try {
    ...
    result = await validationEngagementsService.getPathsById(CONTEXT, PARAMS);
  } catch (error) {
  ...
  if (lodash.isEmpty(ERRORS)) {
    ...
    logger.error(getPathsById.name, { ERRORS });
