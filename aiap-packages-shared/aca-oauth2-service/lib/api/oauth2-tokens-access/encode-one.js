...
*/
const MODULE_ID = 'aca-oauth2-service-oauth2-tokens-access-encode-one';
...
const encodeOne = (context, params) => {
  ...
  try {
    ...
    retVal = jwt.sign(params?.token, context?.user?.session?.tenant?.integration?.tokenAccess?.secret);
    return retVal;
  } catch (error) {
