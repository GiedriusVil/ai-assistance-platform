...
*/
const MODULE_ID = 'aca-ai-service-express-routes-ai-entities-export-many-by-query';
...
export const exportManyByQuery = async (
  ...
  try {
    ...
    const FLATTEN_ENTITIES = await aiEntitiesService.retrieveManyByQueryFlattenByValues(CONTEXT, PARAMS);
    const CSV_FIELDS = [
      'entity',
      'value',
      'type',
    ...
    result = PARSER.parse(FLATTEN_ENTITIES);
  } catch (error) {
  ...
  if (
    ...
    response.setHeader('Content-disposition', 'attachment; filename=Entities_' + currentDateAsString() + '.csv');
