...
*/
const MODULE_ID = 'aca-classifier-service-runtime-data-service-synchronize-with-database';
...
const { synchronizeWithDatabaseClassifierModelsByTenant } = require('./synchronize-with-database-classifier-models-by-tenant');
...
const _synchronizeWithDatabaseByTenant = async (context, params) => {
  ...
  try {
    ...
    const CLASSIFIER_MODELS = await synchronizeWithDatabaseClassifierModelsByTenant(CONTEXT, params);
    const RET_VAL = {
      tenantId: params?.tenantId,
      classifierModels: CLASSIFIER_MODELS,
    };
    return RET_VAL;
  } catch (error) {
    ...
    if (
      ...
      if (
        ...
        for (let result of PROMISES_RESULTS) {
          logger.info(`TenantCustomizer ClassifierModels synchronized [TenantId: ${result?.tenantId}, QTY.: ${result?.classifierModels?.length}]`);
        }
      }
    }
  } catch (error) {
