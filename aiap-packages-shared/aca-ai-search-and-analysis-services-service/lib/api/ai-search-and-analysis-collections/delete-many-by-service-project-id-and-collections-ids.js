...
*/
const MODULE_ID = 'aca-ai-search-and-analysis-services-service-ai-search-and-analysis-collections-delete-many-by-service-project-id-and-collections-ids';
...
const deleteManyByServiceProjectIdAndCollectionIds = async (context, params) => {
  ...
  try {
    ...
    const DELETE_PROMISES = AI_SEARCH_AND_ANALYSIS_COLLECTIONS?.map((aiSearchAndAnalysisCollection) => {
      return AI_SEARCH_AND_ANALYSIS_SERVICE_ADAPTER.collections.deleteOne(context, { 
    ...
    const RET_VAL = await DATASOURCE.aiSearchAndAnalysisCollections.deleteManyByIds(context, params);
    return RET_VAL;
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error(deleteManyByServiceProjectIdAndCollectionIds.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
};
