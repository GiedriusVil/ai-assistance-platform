...
*/
const MODULE_ID = 'app-service-runtime-data-service-synchronize-with-config-directory-access-group';
...
export const synchronizeWithConfigDirectoryAccessGroup = async (
  context: IContextV1,
  params: {
  ...
  try {
    ...
    if (
      ...
      const FILE_PATH = `${configAbsolutePath}/runtime-data-local/portal/access-groups/${params?.value?.id}.json`;
    ...
    logger.error(synchronizeWithConfigDirectoryAccessGroup.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}
