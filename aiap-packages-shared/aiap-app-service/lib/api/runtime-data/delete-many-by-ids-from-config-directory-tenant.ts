...
*/
const MODULE_ID = 'app-service-runtime-data-service-delete-many-by-ids-from-config-directory-tenant';
...
export const deleteManyByIdsFromConfigDirectoryTenant = async (
  context: IContextV1,
  params: {
    ids: Array<any>,
  },
  ...
  try {
      ...
      if (
        ...
        for (const TMP_ID of params.ids) {
          if (
            ...
            const FILE_PATH = `${configAbsolutePath}/runtime-data-local/portal/tenants/${TMP_ID}.json`;
    ...
    logger.error(deleteManyByIdsFromConfigDirectoryTenant.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}
