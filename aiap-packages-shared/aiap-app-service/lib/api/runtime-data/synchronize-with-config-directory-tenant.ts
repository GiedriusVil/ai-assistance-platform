...
*/
const MODULE_ID = 'app-service-runtime-data-service-synchronize-with-config-directory-tenant';
...
export const synchronizeWithConfigDirectoryTenant = async (
  context: IContextV1,
  params: {
    value: ITenantV1,
  },
  ...
  try {
    ...
    if (
      ...
      const FILE_PATH = `${configAbsolutePath}/runtime-data-local/portal/tenants/${params?.value?.id}.json`;
    ...
    logger.error(synchronizeWithConfigDirectoryTenant.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}
