...
*/
const MODULE_ID = 'aca-topic-modeling-express-routes-controller-topics-export-many';
...
const _constructFileName = (context) => {
  try {
    ...
    const RET_VAL = `[${TENANT_ID}]topic_miner_topics.${currentDateAsString()}.json`;
    return RET_VAL;
  } catch (error) {
...
const exportMany = async (request, response) => {
  ...
  try {
    ...
    PARAMS.jobId = request?.query?.jobId;
    if (lodash.isArray(request?.query?.ids)) {
      PARAMS.ids = request?.query?.ids;
    } else if (lodash.isString(request?.query?.ids)) {
      PARAMS.ids = [request?.query?.ids];
    }
    const DATA = await topicModelingService.exportMany(CONTEXT, PARAMS);

    if (
      lodash.isEmpty(DATA)
    ) {
      const MESSAGE = `Unable to find topics!`;
