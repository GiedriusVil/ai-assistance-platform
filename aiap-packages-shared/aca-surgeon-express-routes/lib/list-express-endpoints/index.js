...
const getRouteMiddlewares = function (route) {
  const RET_VAL = route.stack.map((item) => {
...
const parseExpressRoute = function (route, basePath) {
  ...
  const endpoints = paths.map((path) => {
    const COMPLETE_PATH = basePath && path === '/' ?
      basePath :
      `${basePath}${path}`;


    const COMPLETE_METHODS = getRouteMethods(route);
    const COMPLETE_MIDDLEWARES = getRouteMiddlewares(route);
    if (
      '/api/v1/classification-catalog/catalogs/:id' === COMPLETE_PATH
    ) {
      console.log('/api/v1/classification-catalog/catalogs/:id', {
        COMPLETE_METHODS,
        COMPLETE_MIDDLEWARES,
      });
    }
    ...
    const endpoint = {
      ...
      middlewares: COMPLETE_MIDDLEWARES,
    };
...
const addEndpoints = function (currentEndpoints, endpointsToAdd) {
  ...
  endpointsToAdd.forEach((newEndpoint) => {
    const existingEndpoint = currentEndpoints
      .find((item) => item?.path === newEndpoint.path && newEndpoint?.methods === item?.methods);

    if (existingEndpoint !== undefined) {

      const newMethods = newEndpoint
        .methods
        .filter((method) => !existingEndpoint.methods.includes(method));
