...
*/
const MODULE_ID = 'aca-ai-search-and-analysis-services-express-routes-controllers-ai-search-and-analysis-projects-export-many-by-query';
...
const _constructFileName = (context) => {
  try {
    ...
    const RET_VAL = `[${TENANT_ID}]ai-search-and-analysis-projects.${currentDateAsString()}.json`;
    return RET_VAL;
  } catch (error) {
...
const exportManyByQuery = async (request, response) => {
  ...
  let result;
  try {
    const RESULT = await aiSearchAndAnalysisProjectsService.findManyByQuery(CONTEXT, PARAMS);
    const ITEMS = RESULT?.items;
    if (
      lodash.isEmpty(ITEMS)
    ) {
      const MESSAGE = `Unable to find ai-search-and-analysis-projects!`;
