...
const findWithUserInteractionHourGroups = async (request, response) => {
  const ERRORS = [];
  let context;
  let contextUserId;
  let query;
  ...
  try {
    ...
    result = await conversationsService.findWithUserInteractionHourGroups(context, params);
  } catch (error) {
  ...
  if (
    ...
    logger.error(`${findWithUserInteractionHourGroups.name}`, { errors: ERRORS });
