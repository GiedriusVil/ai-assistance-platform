...
*/
const MODULE_ID = 'aca-analytics-live-express-routes-controllers-messages-find-transfer-totals-by-query-hour-groups';
...
const { messagesService } = require('@ibm-aca/aca-analytics-live-service');

const findTransferTotalsByQueryHourGroups = async (request, response) => {
  const ERRORS = [];
  let context;
  let contextUserId;
  let params;
  ...
  try {
    ...
    if (lodash.isEmpty(ERRORS)) {
      result = await messagesService.findTransferTotalsByQueryHourGroups(context, params);
    }
  } catch (error) {
  ...
  if (lodash.isEmpty(ERRORS)) {
    ...
    logger.error(`${findTransferTotalsByQueryHourGroups.name}`, { errors: ERRORS });
