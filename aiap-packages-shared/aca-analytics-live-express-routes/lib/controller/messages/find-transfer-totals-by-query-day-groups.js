...
*/
const MODULE_ID = 'aca-analytics-live-express-routes-controllers-messages-find-transfer-totals-by-query-day-groups';
...
const findTransferTotalsByQueryDayGroups = async (request, response) => {
  const ERRORS = [];
  let context;
  let contextUserId;
  let params;
  ...
  try {
    ...
    if (lodash.isEmpty(ERRORS)) {
      result = await messagesService.findTransferTotalsByQueryDayGroups(context, params);
    }
  } catch (error) {
  ...
  if (lodash.isEmpty(ERRORS)) {
    ...
    logger.error(`${findTransferTotalsByQueryDayGroups.name}`, { errors: ERRORS });
