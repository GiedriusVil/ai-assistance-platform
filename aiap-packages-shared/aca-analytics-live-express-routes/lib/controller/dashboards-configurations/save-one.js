...
*/
const MODULE_ID = 'aca-analytics-live-express-routes-controllers-dashboards-configurations-save-one';
...
const saveOne = async (request, response) => {
  ...
  const DASHBOARDS_CONFIGURATIONS = ramda.path(['body'], request);
  const PARAMS = { value: DASHBOARDS_CONFIGURATIONS };
  let result;
  try {
    if (
    ...
    if (lodash.isEmpty(ERRORS)) {
      ...
      result = await dashboardsConfigurationsService.saveOne(CONTEXT, PARAMS);
    }
  } catch (error) {
