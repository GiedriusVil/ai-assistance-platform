...
*/
const MODULE_ID = 'aca-live-analytics-charts-service-delete-many-by-ids';
...
const retrieveRefsByIds = async (context, ids) => {
  try {
    ...
    const PROMISES = [];
    for (let id of ids) {
      PROMISES.push(await DATASOURCE.charts.findOneById(context, { id: id }));
    }
...
const deleteManyByIds = async (context, params) => {
  ...
  try {
    ...
    await runtimeDataService.deleteManyByIdsFromDirectoryLiveAnalyticsChartsService(context, { ids: REFS });
    const RET_VAL = await DATASOURCE.charts.deleteManyByIds(context, params);
