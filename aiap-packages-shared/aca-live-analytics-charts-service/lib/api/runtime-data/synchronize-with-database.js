...
*/
const MODULE_ID = 'aca-live-analytics-charts-runtime-data-service-synchronize-with-database';
...
const { synchronizeWithDatabaseLiveAnalyticsChartsByTenant } = require('./synchronize-with-database-live-analytics-charts-by-tenant');
...
const _synchronizeWithDatabaseByTenant = async (context, params) => {
  ...
  try {
    ...
    const CHARTS = await synchronizeWithDatabaseLiveAnalyticsChartsByTenant(CONTEXT, params);
    const RET_VAL = {
    ...
    if (
      ...
      if (
        ...
        for (let result of PROMISES_RESULTS) {
          logger.info(`LiveAnalytics charts synchronized [TenantID: ${result?.tenantId}, QTY.: ${result?.value?.length}]`);
        }
      }
    }
  } catch (error) {
