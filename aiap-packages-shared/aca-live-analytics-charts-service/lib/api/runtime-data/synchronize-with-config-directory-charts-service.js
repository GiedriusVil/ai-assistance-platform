...
*/
const MODULE_ID = 'aca-live-analytics-service-runtime-data-service-synchronize-with-config-directory-live-analytics-service';
...
const synchronizeWithConfigDirectoryLiveAnalyticsChartsService = async (context, params) => {
  ...
  try {
    ...
    if (
      ...
      let filePath = `${configAbsolutePath}/runtime-data-local/live-analytics/${context?.user?.session?.tenant?.id}/charts/${params?.value?.ref}.json`;
      fsExtra.ensureDirSync(`${configAbsolutePath}/runtime-data-local/live-analytics/${context?.user?.session?.tenant?.id}/charts`);
    ...
    logger.error(synchronizeWithConfigDirectoryLiveAnalyticsChartsService.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}
