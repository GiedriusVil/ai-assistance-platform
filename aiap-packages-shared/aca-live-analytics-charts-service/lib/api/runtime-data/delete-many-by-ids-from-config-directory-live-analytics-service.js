...
*/
const MODULE_ID = 'aca-engagements-service-runtime-data-service-delete-many-from-ids-from-config-directory-charts-service';
...
const deleteManyByIdsFromDirectoryLiveAnalyticsChartsService = async (context, params) => {
  ...
  try {
    ...
    if (
      ...
      const DIR_CHARTS= `${configAbsolutePath}/runtime-data-local/live-analytics/${context?.user?.session?.tenant?.id}/charts`;
      fsExtra.ensureDirSync(DIR_CHARTS);
      ...
      if (
        ...
        for (let tmpId of params.ids) {
          if (
            ...
            fsExtra.removeSync(`${DIR_CHARTS}/${tmpId}.json`);
          }
        }
      }
    }
    ...
    logger.error(deleteManyByIdsFromDirectoryLiveAnalyticsChartsService.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}
