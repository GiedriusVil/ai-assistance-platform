...
*/
const MODULE_ID = 'aca-live-analytics-filters-runtime-data-service-synchronize-with-database';
...
const { synchronizeWithDatabaseLiveAnalyticsFiltersByTenant } = require('./synchronize-with-database-live-analytics-filters-by-tenant');
...
const _synchronizeWithDatabaseByTenant = async (context, params) => {
  ...
  try {
    ...
    const FILTERS = await synchronizeWithDatabaseLiveAnalyticsFiltersByTenant(CONTEXT, params);
    const RET_VAL = {
    ...
    if (
      ...
      if (
        ...
        for (let result of PROMISES_RESULTS) {
          logger.info(`LiveAnalytics filters synchronized [TenantID: ${result?.tenantId}, QTY.: ${result?.value?.length}]`);
        }
      }
    }
  } catch (error) {
