...
*/
const MODULE_ID = 'aca-live-analytics-service-runtime-data-service-synchronize-with-config-directory-filters-service';
...
const synchronizeWithConfigDirectoryLiveAnalyticsFiltersService = async (context, params) => {
  ...
  try {
    ...
    if (
      ...
      let filePath = `${configAbsolutePath}/runtime-data-local/live-analytics/${context?.user?.session?.tenant?.id}/filters/${params?.value?.ref}.json`;
      fsExtra.ensureDirSync(`${configAbsolutePath}/runtime-data-local/live-analytics/${context?.user?.session?.tenant?.id}/filters`);
      ...
      if (
        ...
        fsExtra.writeFileSync(
          `${configAbsolutePath}/runtime-data-local/live-analytics/${context?.user?.session?.tenant?.id}/filters/${params?.value?.ref}.js`,
          TMP_VALUE?.code
        );
    ...
    logger.error(synchronizeWithConfigDirectoryLiveAnalyticsFiltersService.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}
