...
*/
const MODULE_ID = 'aca-classifier-datasource-provider-index';
...
const { createAcaClassifierDatasources } = require('./lib/datasource-factory');
...
const initManyByRootConfiguration = async (path = DEFAULT_CONFIG_PATH) => {
  ...
  if (
    ...
    const DATASOURCES = await createAcaClassifierDatasources(CONFIG_DATASOURCES);
...
const initManyByTenant = async (params) => {
  ...
  try {
    const CONFIG_DATASOURCES_CONVERSATIONS = lodash.filter(CONFIG_DATASOURCES_ALL, { type: 'classifier' });
    if (
      !lodash.isEmpty(CONFIG_DATASOURCES_CONVERSATIONS)
    ) {
      const DATASOURCES = await createAcaClassifierDatasources(CONFIG_DATASOURCES_CONVERSATIONS);
...
const getAcaClassifierDatasourceByContext = (context) => {
  ...
  const RET_VAL = getAcaClassifierDatasourceByTenant(TENANT);
  return RET_VAL;
}

const getAcaClassifierDatasourceByTenant = (tenant) => {
  const DATASOURCES_CONFIG = tenant?._datasources;
  let retVal;
  if (
    lodash.isArray(DATASOURCES_CONFIG)
  ) {
    const DATASOURCE_INDEX = lodash.findIndex(DATASOURCES_CONFIG, { type: 'classifier' });
    if (
      DATASOURCE_INDEX >= 0
    ) {
      ...
      retVal = getAcaClassifierDatasource(DATASOURCE_ID, DATASOURCE_HASH);
    }
  }
  return retVal;
}

const getAcaClassifierDatasource = (id = 'default', hash = undefined) => {
  let retVal;
  if (
    !lodash.isEmpty(id) &&
    !lodash.isEmpty(hash)
...
const getAcaClassifierDatasources = (context) => {
...
module.exports = {
  ...
  initManyByTenant,
  getAcaClassifierDatasources,
  getAcaClassifierDatasource,
  getAcaClassifierDatasourceByContext,
