...
*/
const MODULE_ID = 'aca-classifier-datasource-provider-datasource-factory';
...
const { AcaClassifierDatasourceMongo } = require('../datasource-mongo');
...
const _createMongoDatasource = async (configuration) => {
  try {
    ...
    const RET_VAL = new AcaClassifierDatasourceMongo(configuration);
...
const createAcaClassifierDatasource = async (configuration) => {
  try {
    ...
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error(createAcaClassifierDatasource.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

const createAcaClassifierDatasources = async (configurations) => {
  try {
    if (
    ...
    const PROMISES = configurations.map((configuration) => {
      return createAcaClassifierDatasource(configuration);
    });
    const RET_VAL = await Promise.all(PROMISES);
    return RET_VAL;
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error(createAcaClassifierDatasources.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

module.exports = {
  createAcaClassifierDatasource,
  createAcaClassifierDatasources,
}
