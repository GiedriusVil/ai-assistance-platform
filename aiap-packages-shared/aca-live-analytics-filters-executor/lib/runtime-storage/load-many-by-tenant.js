...
*/
const MODULE_ID = 'aca-live-analytics-filters-executor-runtime-storage-load-many-by-tenant';
...
const loadManyByTenant = async (params) => {
  ...
  try {
    ...
    const RESPONSE = await DATASOURCE.filters.findManyByQuery({}, PARAMS);
    const FILTERS_DEFS = RESPONSE?.items;
    let retVal;
    if (
      lodash.isArray(FILTERS_DEFS)
    ) {
      const PROMISES = [];
      for (let filterDef of FILTERS_DEFS) {
        let tmpParams = {
          tenant: TENANT,
          ref: filterDef?.ref,
