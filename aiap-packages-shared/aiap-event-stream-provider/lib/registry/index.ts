...
*/
const MODULE_ID = 'aiap-event-stream-provider-event-stream-registry';
...
const addOneToRegistryById = (
  id: string,
  client: EventStreamV1<IEventStreamConfigurationV1>,
): void => {
  try {
    if (
      lodash.isEmpty(id)
...
const addManyToRegistryByIdAndHash = (
  clients: Array<EventStreamV1<IEventStreamConfigurationV1>>,
): void => {
  try {
    if (
    ...
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error('addManyToRegistryByIdAndHash', { ACA_ERROR });
    throw ACA_ERROR;
  }
}
...
const getOneById = (
  id: string,
): EventStreamV1<IEventStreamConfigurationV1> => {
  try {
    if (
      lodash.isEmpty(id)
    ) {
