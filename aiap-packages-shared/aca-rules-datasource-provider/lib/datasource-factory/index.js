...
*/
const MODULE_ID = 'aca-rules-datasource-provider-datasource-factory';
...
const { AcaRulesDatasourceMongo } = require('../datasource-mongo');
...
const _createMongoDatasource = async (configuration) => {
    try {
        ...
        const RET_VAL = new AcaRulesDatasourceMongo(configuration);
...
const createAcaRulesDatasource = async (configuration) => {
    try {
        ...
        const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
        logger.error('createAcaRulesDatasource', { ACA_ERROR });
        throw ACA_ERROR;
    }
}

const createAcaRulesDatasources = async (configurations) => {
    try {
        if (
        ...
        const PROMISES = configurations.map((configuration) => {
            return createAcaRulesDatasource(configuration);
        });
        const RET_VAL = await Promise.all(PROMISES);
        return RET_VAL;
    } catch (error) {
        const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
        logger.error('createAcaRulesDatasources', { ACA_ERROR });
        throw ACA_ERROR;
    }
}

module.exports = {
    createAcaRulesDatasource,
    createAcaRulesDatasources,
}
