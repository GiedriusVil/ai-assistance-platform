...
const findLiteOneById = async (datasource, context, params) => {
  ...
  try {
    ...
    const CATALOGS = await ACA_MONGO_CLIENT
    ...
    const CATALOG = ramda.path([0], CATALOGS);
    sanitizeIdAttribute(CATALOG);
    const RET_VAL = transformToLiteClassificationCatalogCategory(CATALOG, PARAMS_LANGUAGE);
    return RET_VAL;
  } catch (error) {
