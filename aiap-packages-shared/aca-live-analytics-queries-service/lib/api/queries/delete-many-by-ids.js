...
*/
const MODULE_ID = 'aca-live-analytics-service-queries-delete-many-by-ids';
...
const publishDeleteEvents = (context, refs) => {
  try {
    if (
      ...
      for (let ref of refs) {
        getEventStreamByContext(context).publish(AIAP_EVENT_TYPE.DELETE_LIVE_ANALYTICS_QUERY, { ref });
      }
    }
  } catch (error) {
...
const retrieveRefsByIds = async (context, ids) => {
  try {
    ...
    const PROMISES = [];
    for (let id of ids) {
      PROMISES.push(await DATASOURCE.queries.findOneById(context, { id: id }));
    }
    const RESULT = await Promise.all(PROMISES);
    const RET_VAL = RESULT.map(q => { return q?.ref })
    return RET_VAL;
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
...
const deleteManyByIds = async (context, params) => {
  ...
  try {
    ...
    const RET_VAL = await DATASOURCE.queries.deleteManyByIds(context, params);
    ...
    await runtimeDataService.deleteManyByIdsFromConfigDirectoryQuery(context, { ids: REFS });
