...
*/
const MODULE_ID = 'aca-live-analytics-queries-service-runtime-data-service-synchronize-with-database';
...
const _synchronizeWithDatabaseByTenant = async (context, params) => {
  ...
  try {
    ...
    const QUERIES = await synchronizeWithDatabaseQueriesByTenant(CONTEXT, params);
    const RET_VAL = {
    ...
    if (
      ...
      if (
        ...
        for (let result of PROMISES_RESULTS) {
          logger.info(`LiveAnalytics queries synchronized [TenantID: ${result?.tenantId}, QTY.: ${result?.value?.length}]`);
        }
      }
    }
  } catch (error) {
