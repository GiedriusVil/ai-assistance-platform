...
*/
const MODULE_ID = 'aca-ai-search-and-analysis-service-adapter-provider-adapter-watson-discovery-v2-documents-delete-one';
...
const deleteOne = async (context, params) => {
  ...
  const AI_SEARCH_AND_ANALYSIS_DOCUMENT = params?.aiSearchAndAnalysisDocument;
  try {
    ...
    if (lodash.isEmpty(AI_SEARCH_AND_ANALYSIS_DOCUMENT)) {
      const MESSAGE = 'Missing required params.aiSearchAndAnalysisCollection parameter';
      throwAcaError(MODULE_ID, ACA_ERROR_TYPE.SYSTEM_ERROR, MESSAGE);
    }
    ...
    const PARAMS = {
      ...
      documentId: AI_SEARCH_AND_ANALYSIS_DOCUMENT?.external?.document_id,
    };

    try {
      await WATSON_DISCOVERY_SERVICE.documents.deleteOne(context, PARAMS);
    } catch (error) {
      if (error?.external?.code !== 404) {
        throw error;
      }
      logger.warn(`${deleteOne.name} Document not found.`, { error });
    }
