...
*/
const MODULE_ID = 'aca-ai-search-and-analysis-service-adapter-provider-adapter-watson-discovery-v2-projects-delete-one';
...
const deleteOne = async (context, params) => {
  ...
  try {
    ...
    try {
      await WATSON_DISCOVERY_SERVICE.projects.deleteOne(context, PARAMS);
    } catch (error) {
      if (error?.external?.code !== 404) {
        throw error;
      }
      logger.warn(`${deleteOne.name} Project not found.`, { error });
    }
