...
*/
const MODULE_ID = 'aca-rules-conditions-service-runtime-data-synchronize-with-config-directory';
...
const synchronizeWithConfigDirectoryRulesConditions = async (context, params) => {
  ...
  try {
    ...
    if (
      ...
      const DIR_RULES_CONDITIONS = `${configAbsolutePath}/runtime-data-local/policy-manager/${context?.user?.session?.tenant?.id}/rules-v2-conditions`;
      fsExtra.ensureDirSync(DIR_RULES_CONDITIONS);

      fsExtra.writeFileSync(
        `${DIR_RULES_CONDITIONS}/${params?.value?.id}.json`,
    ...
    logger.error(synchronizeWithConfigDirectoryRulesConditions.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}
