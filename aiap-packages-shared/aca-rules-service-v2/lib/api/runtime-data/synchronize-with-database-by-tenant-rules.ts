...
*/
const MODULE_ID = 'aca-rules-service-v2-runtime-data-synchronize-with-database-by-tenant';
...
const synchronizeWithDatabaseByTenantRules = async (context, params) => {
  ...
  try {
    ...
    configPolicyManagerRulesAbsolutePath = `${params?.configPolicyManagerAbsolutePath}/${params?.tenantId}/rules-v2`;
    ...
    if (
      ...
      for (const RULES_FILE of RULES_FILES) {
        if (
          ...
          PROMISES.push(
            DATASOURCE.rules.saveOne(context, { ruleV2: RULE })
          );
        }
      }
    }
