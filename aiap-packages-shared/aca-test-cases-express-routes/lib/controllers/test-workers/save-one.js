...
*/
const MODULE_ID = 'aca-test-cases-express-routes-controller-test-workers-save-one'
...
const saveOne = async (request, response) => {
  ...
  try {
    ...
    logger.info(saveOne.name, {
      context: transformContextForLogger(CONTEXT),
      params: PARAMS,
    });
    result = await testWorkersService.saveOne(CONTEXT, PARAMS);
  } catch (error) {
