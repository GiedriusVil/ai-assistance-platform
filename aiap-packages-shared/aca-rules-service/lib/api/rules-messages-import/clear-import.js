...
*/
const MODULE_ID = 'aca-rules-service-rules-messages-import-clear-import';
...
const _clearImport = async (context, params) => {
  ...
  const IMPORT_MESSAGES = await findManyByQuery(context, QUERY);
  const IDS = IMPORT_MESSAGES?.items?.map(el => el.id) || [];
  ...
  if (!lodash.isEmpty(IDS)) {
    ...
    await DATASOURCE.rulesMessages.deleteManyByIds(context, PARAMS);
  }
