...
*/
const MODULE_ID = 'aca-rules-messages-service-runtime-data-synchronize-with-config-directory';
...
const synchronizeWithConfigDirectoryRulesMessages = async (context, params) => {
  ...
  try {
    ...
    if (
      ...
      const DIR_RULES_MESSAGES = `${configAbsolutePath}/runtime-data-local/policy-manager/${context?.user?.session?.tenant?.id}/rules-messages`;
      fsExtra.ensureDirSync(DIR_RULES_MESSAGES);

      fsExtra.writeFileSync(
        `${DIR_RULES_MESSAGES}/${params?.value?.id}.json`,
    ...
    logger.error(synchronizeWithConfigDirectoryRulesMessages.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

export {
  synchronizeWithConfigDirectoryRulesMessages,
}
