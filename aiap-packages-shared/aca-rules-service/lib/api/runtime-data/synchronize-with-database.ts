...
import { synchronizeWithDatabaseByTenantRulesMessages } from './synchronize-with-database-by-tenant-rules-messages';
...
const _synchronizeWithDatabaseByTenant = async (context, params) => {
  ...
  try {
    ...
    const RULES_MESSAGES = await synchronizeWithDatabaseByTenantRulesMessages(CONTEXT, params);
    const RET_VAL = {
      tenantId: params?.tenantId,
      rules: RULES,
      rulesMessages: RULES_MESSAGES,
    };
    return RET_VAL;
  } catch (error) {
    ...
    if (
      ...
      if (
        ...
        for (const RESULT of PROMISES_RESULTS) {
          ...
          logger.info(`Policy Manager Organizations synchronized [TenantId: ${RESULT?.tenantId}, QTY.: ${RESULT?.rulesMessages?.length}]`);
        }
      }
    }
  } catch (error) {
