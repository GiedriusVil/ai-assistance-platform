...
const deleteManyByIdsFromConfigDirectoryRules = async (context, params) => {
  ...
  try {
    ...
    if (
      ...
      const DIR_RULES = `${configAbsolutePath}/runtime-data-local/policy-manager/${context?.user?.session?.tenant?.id}/rules`;
    ...
    logger.error(deleteManyByIdsFromConfigDirectoryRules.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}
