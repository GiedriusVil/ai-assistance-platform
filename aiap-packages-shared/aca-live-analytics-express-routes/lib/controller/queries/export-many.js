...
*/
const MODULE_ID = 'aca-live-analytics-express-routes-controller-queries-export-many';
...
const _constructFileName = (context) => {
  try {
    ...
    const RET_VAL = `[${TENANT_ID}]queries.${currentDateAsString()}.json`;
    return RET_VAL;
  } catch (error) {
...
const exportMany = async (request, response) => {
  ...
  try {
    ...
    const DATA = await liveAnalyticsQueriesService.exportMany(CONTEXT, PARAMS);
    if (
      lodash.isEmpty(DATA)
    ) {
