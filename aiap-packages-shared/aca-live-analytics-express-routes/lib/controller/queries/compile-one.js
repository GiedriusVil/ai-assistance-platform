/*
  © Copyright IBM Corporation 2022. All Rights Reserved 
   
  SPDX-License-Identifier: EPL-2.0
*/
const MODULE_ID = 'aca-live-analytics-express-routes-queries-compile-one'
const logger = require('@ibm-aca/aca-common-logger')(MODULE_ID);

const lodash = require('@ibm-aca/aca-wrapper-lodash');

const { transformContextForLogger } = require('@ibm-aca/aca-data-transformer');
const { formatIntoAcaError, throwAcaError, ACA_ERROR_TYPE } = require('@ibm-aca/aca-utils-errors');
const { constructActionContextFromRequest } = require('@ibm-aiap/aiap-utils-express-routes');

const { liveAnalyticsQueriesService } = require('@ibm-aca/aca-live-analytics-queries-service');

const compileOne = async (request, response) => {
  const ERRORS = [];
  let context;
  let params;
  let result;
  try {
    context = constructActionContextFromRequest(request);
    params = request?.body;
    const QUERY_ID = params?.id;
    if (
      lodash.isEmpty(QUERY_ID)
    ) {
      const MESSAGE = 'Missing required request.body.id attribute!';
      throwAcaError(MODULE_ID, ACA_ERROR_TYPE.VALIDATION_ERROR, MESSAGE);
    }
    if (lodash.isEmpty(ERRORS)) {
      logger.info('->', {
        context: transformContextForLogger(context),
        params: params,
      });
      result = await liveAnalyticsQueriesService.compileOne(context, params);
    }
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    ACA_ERROR.params = params;
    ERRORS.push(ACA_ERROR);
  }
  if (lodash.isEmpty(ERRORS)) {
    response.status(200).json(result);
  } else {
    logger.error('->', { ERRORS });
    response.status(500).json({ errors: ERRORS });
  }
};

module.exports = {
  compileOne,
};
