...
*/
const MODULE_ID = 'aca-live-analytics-express-routes-controller-tiles-export-many';
...
const { liveAnalyticsTilesService } = require('@ibm-aca/aca-live-analytics-tiles-service');
...
const _constructFileName = (context) => {
  try {
    ...
    const RET_VAL = `[${TENANT_ID}]tiles.${currentDateAsString()}.json`;
    return RET_VAL;
  } catch (error) {
...
const exportMany = async (request, response) => {
  ...
  try {
    ...
    const DATA = await liveAnalyticsTilesService.exportMany(CONTEXT, PARAMS);
    if (
      lodash.isEmpty(DATA)
    ) {
      const MESSAGE = `Unable to find tiles!`;
