...
*/
const MODULE_ID = 'aca-live-analytics-express-routes-controller-charts-export-many';
...
const { liveAnalyticsChartsService } = require('@ibm-aca/aca-live-analytics-charts-service');
...
const _constructFileName = (context) => {
  try {
    ...
    const RET_VAL = `[${TENANT_ID}]charts.${currentDateAsString()}.json`;
    return RET_VAL;
  } catch (error) {
...
const exportMany = async (request, response) => {
  ...
  try {
    ...
    const DATA = await liveAnalyticsChartsService.exportMany(CONTEXT, PARAMS);
    if (
      lodash.isEmpty(DATA)
    ) {
      const MESSAGE = `Unable to find charts!`;
