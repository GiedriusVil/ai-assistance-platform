...
*/
const MODULE_ID = 'aca-live-analytics-express-routes-controller-filters-export-many';
...
const _constructFileName = (context) => {
  try {
    ...
    const RET_VAL = `[${TENANT_ID}]filters.${currentDateAsString()}.json`;
    return RET_VAL;
  } catch (error) {
...
const exportMany = async (request, response) => {
  ...
  try {
    ...
    const DATA = await liveAnalyticsFiltersService.exportMany(CONTEXT, PARAMS);
    if (
      lodash.isEmpty(DATA)
    ) {
      const MESSAGE = `Unable to find queries!`;
