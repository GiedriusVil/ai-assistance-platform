...
*/
const MODULE_ID = 'aca-metrics-datasource-provider-datasource-factory';
...
const { AcaMetricsDatasourceMongo } = require('../datasource-mongo');
...
const _createMongoDatasource = async (configuration) => {
    try {
        ...
        const RET_VAL = new AcaMetricsDatasourceMongo(configuration);
...
const createAcaMetricsDatasource = async (configuration) => {
    try {
        ...
        const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
        logger.error('createAcaMetricsDatasource', { ACA_ERROR });
        throw ACA_ERROR;
    }
}

const createAcaMetricsDatasources = async (configurations) => {
    try {
        if (
        ...
        const PROMISES = configurations.map((configuration) => {
            return createAcaMetricsDatasource(configuration);
        });
        const RET_VAL = await Promise.all(PROMISES);
        return RET_VAL;
    } catch (error) {
        const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
        logger.error('createAcaMetricsDatasources', { ACA_ERROR });
        throw ACA_ERROR;
    }
}

module.exports = {
    createAcaMetricsDatasource,
    createAcaMetricsDatasources,
}
