...
*/
const MODULE_ID = 'aca-metrics-datasource-provider-index';
...
const { createAcaMetricsDatasources } = require('./lib/datasource-factory');
...
const initManyByRootConfiguration = async (path = DEFAULT_CONFIG_PATH) => {
  ...
  if (
    ...
    const DATASOURCES = await createAcaMetricsDatasources(CONFIG_DATASOURCES);
...
const initManyByTenant = async (params) => {
  ...
  try {
    ...
    const CONFIG_DATASOURCES_METRICS = lodash.filter(CONFIG_DATASOURCES_ALL, { type: 'metrics' });
    if (
      !lodash.isEmpty(CONFIG_DATASOURCES_METRICS)
    ) {
      const DATASOURCES = await createAcaMetricsDatasources(CONFIG_DATASOURCES_METRICS);
...
const getAcaMetricsDatasourceByContext = (context) => {
  ...
  const RET_VAL = getAcaMetricsDatasourceByTenant(TENANT);
  return RET_VAL;
}

const getAcaMetricsDatasourceByTenant = (tenant) => {
  const DATASOURCES_CONFIG = ramda.path(['_datasources'], tenant);
  let retVal;
  if (
    lodash.isArray(DATASOURCES_CONFIG)
  ) {
    const DATASOURCE_INDEX = lodash.findIndex(DATASOURCES_CONFIG, { type: 'metrics' })
    if (
      DATASOURCE_INDEX >= 0
    ) {
      ...
      retVal = getAcaMetricsDatasource(DATASOURCE_ID, DATASOURCE_HASH);
    }
  }
  return retVal;
}

const getAcaMetricsDatasource = (id = 'default', hash = undefined) => {
  let retVal;
  if (
    !lodash.isEmpty(id) &&
    !lodash.isEmpty(hash)
...
const getAcaMetricsDatasources = (context) => {
...
module.exports = {
  ...
  initManyByTenant,
  getAcaMetricsDatasources,
  getAcaMetricsDatasource,
  getAcaMetricsDatasourceByContext,
  getAcaMetricsDatasourceByTenant,
}
