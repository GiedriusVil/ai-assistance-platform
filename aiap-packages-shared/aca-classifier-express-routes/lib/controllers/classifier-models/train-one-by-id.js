...
*/
const MODULE_ID = 'aca-classifier-express-routes-controller-classifier-models-train-one-by-id';
...
const trainOneById = async (request, response) => {
  ...
  try {
    ...
    const PARAMS = { id: ID };
    const CONTEXT = constructActionContextFromRequest(request);
    retVal = await classifierService.trainOneById(CONTEXT, PARAMS);
  } catch (error) {
