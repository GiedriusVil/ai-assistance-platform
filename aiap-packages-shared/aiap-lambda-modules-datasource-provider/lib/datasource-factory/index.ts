...
*/
const MODULE_ID = 'aiap-lambda-modules-datasource-provider-datasource-factory';
...
import { LambdaModulesDatasourceMongoV1 } from '../datasource-mongo';
...
const _createMongoDatasource = async (configuration) => {
  try {
    ...
    const RET_VAL = new LambdaModulesDatasourceMongoV1(configuration);
...
const createAcaLambdaModulesDatasource = async (configuration) => {
  try {
    ...
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error('createAcaLambdaModulesDatasource', { ACA_ERROR });
    throw ACA_ERROR;
  }
}

const createAcaLambdaModulesDatasources = async (configurations) => {
  try {
    if (
    ...
    const PROMISES = configurations.map((configuration) => {
      return createAcaLambdaModulesDatasource(configuration);
    });
    const RET_VAL = await Promise.all(PROMISES);
    return RET_VAL;
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error('createAcaLambdaModulesDatasources', { ACA_ERROR });
    throw ACA_ERROR;
  }
}

export {
  createAcaLambdaModulesDatasource,
  createAcaLambdaModulesDatasources,
}
