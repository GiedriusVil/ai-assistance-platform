...
*/
const MODULE_ID = `aiap-mongo-client-provider-client-delete-many`;
...
const _deleteMany = async (
  ...
  try {
    ...
    const RET_VAL = await DB
      ...
      .deleteMany(PARAMS_FILTER, PARAMS_OPTIONS);
    ...
    logger.error(_deleteMany.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}
...
const deleteMany = (
  ...
  const RET_VAL = async (bail, attempt): Promise<DeleteResult> => {
    try {
      logMethodParams(deleteMany.name, context, params);
      const RET_VAL = await _deleteMany(client, context, params);
      return RET_VAL;
    } catch (error) {
      const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
