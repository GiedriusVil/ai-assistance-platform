...
*/
const MODULE_ID = 'aca-live-analytics-service-tiles-delete-many-by-ids';
...
const retrieveRefsByIds = async (context, ids) => {
  try {
    ...
    const PROMISES = [];
    for (let id of ids) {
      PROMISES.push(await DATASOURCE.tiles.findOneById(context, { id: id }));
    }
...
const deleteManyByIds = async (context, params) => {
  ...
  try {
    ...
    const RET_VAL = await DATASOURCE.tiles.deleteManyByIds(context, params);
    ...
    await runtimeDataService.deleteManyByIdsFromDirectoryLiveAnalyticsTilesService(context, { ids: REFS });
