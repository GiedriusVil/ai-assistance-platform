...
*/
const MODULE_ID = 'aca-live-analytics-runtime-data-service-delete-many-from-ids-from-config-directory-tiles-service';
...
const deleteManyByIdsFromDirectoryLiveAnalyticsTilesService = async (context, params) => {
  ...
  try {
    ...
    if (
      ...
      const DIR_TILES= `${configAbsolutePath}/runtime-data-local/live-analytics/${context?.user?.session?.tenant?.id}/tiles`;
      fsExtra.ensureDirSync(DIR_TILES);
      ...
      if (
        ...
        for (let tmpId of params.ids) {
          if (
            ...
            fsExtra.removeSync(`${DIR_TILES}/${tmpId}.json`);
          }
        }
      }
    }
    ...
    logger.error(deleteManyByIdsFromDirectoryLiveAnalyticsTilesService.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}
