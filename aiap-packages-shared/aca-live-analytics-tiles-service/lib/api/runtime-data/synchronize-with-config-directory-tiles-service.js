...
*/
const MODULE_ID = 'aca-live-analytics-service-runtime-data-service-synchronize-with-config-directory-tiles-service';
...
const synchronizeWithConfigDirectoryLiveAnalyticsTilesService = async (context, params) => {
  ...
  try {
    ...
    if (
      ...
      let filePath = `${configAbsolutePath}/runtime-data-local/live-analytics/${context?.user?.session?.tenant?.id}/tiles/${params?.value?.ref}.json`;
      fsExtra.ensureDirSync(`${configAbsolutePath}/runtime-data-local/live-analytics/${context?.user?.session?.tenant?.id}/tiles`);
    ...
    logger.error(synchronizeWithConfigDirectoryLiveAnalyticsTilesService.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}
