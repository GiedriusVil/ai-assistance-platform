...
*/
const MODULE_ID = 'aca-live-analytics-tiles-runtime-data-service-synchronize-with-database';
...
const { synchronizeWithDatabaseLiveAnalyticsTilesByTenant } = require('./synchronize-with-database-live-analytics-tiles-by-tenant');
...
const _synchronizeWithDatabaseByTenant = async (context, params) => {
  ...
  try {
    ...
    const TILES = await synchronizeWithDatabaseLiveAnalyticsTilesByTenant(CONTEXT, params);
    const RET_VAL = {
    ...
    if (
      ...
      if (
        ...
        for (let result of PROMISES_RESULTS) {
          logger.info(`LiveAnalytics tiles synchronized [TenantID: ${result?.tenantId}, QTY.: ${result?.value?.length}]`);
        }
      }
    }
  } catch (error) {
