...
*/
const MODULE_ID = 'aca-metrics-express-routes-controller-doc-validations-count-rule-frequency';
...
const countRuleFrequency = async (request, response) => {
  const ERRORS = [];
  ...
  try {
    ...
    result = await docValidationsMetricsService.countRuleFrequency(context, params);
  } catch (error) {
  ...
  if (lodash.isEmpty(ERRORS)) {
    ...
    logger.error(`${countRuleFrequency.name}`, { errors: ERRORS });
