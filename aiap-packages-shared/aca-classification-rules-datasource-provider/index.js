...
*/
const MODULE_ID = 'aca-classification-rules-datasource-provider-index';
...
const { createAcaClassificationRulesDatasources } = require('./lib/datasource-factory');
...
const initManyByRootConfiguration = async (path = DEFAULT_CONFIG_PATH) => {
  ...
  if (
    ...
    const DATASOURCES = await createAcaClassificationRulesDatasources(CONFIG_DATASOURCES);
...
const initManyByTenant = async (params) => {
  ...
  try {
    ...
    const CONFIG_DATASOURCES_CONVERSATIONS = lodash.filter(CONFIG_DATASOURCES_ALL, { type: 'classificationRules' });
    if (
      !lodash.isEmpty(CONFIG_DATASOURCES_CONVERSATIONS)
    ) {

      const DATASOURCES = await createAcaClassificationRulesDatasources(CONFIG_DATASOURCES_CONVERSATIONS);
...
const getAcaClassificationRulesDatasourceByContext = (context) => {
  ...
  const RET_VAL = getAcaClassificationRulesDatasourceByTenant(TENANT);
  return RET_VAL;
}

const getAcaClassificationRulesDatasourceByTenant = (tenant) => {
  const DATASOURCES_CONFIG = ramda.path(['_datasources'], tenant);
  let retVal;
  if (
    lodash.isArray(DATASOURCES_CONFIG)
  ) {
    const DATASOURCE_INDEX = lodash.findIndex(DATASOURCES_CONFIG, { type: 'classificationRules' })
    if (
      DATASOURCE_INDEX >= 0
    ) {
      ...
      retVal = getAcaClassificationRulesDatasource(DATASOURCE_ID, DATASOURCE_HASH);
    }
  }
  return retVal;
}

const getAcaClassificationRulesDatasource = (id = 'default', hash = undefined) => {
  let retVal;
  if (
    !lodash.isEmpty(id) &&
    !lodash.isEmpty(hash)
...
const getAcaClassificationRulesDatasources = (context) => {
...
module.exports = {
  ...
  initManyByTenant,
  getAcaClassificationRulesDatasourceByContext,
  getAcaClassificationRulesDatasourceByTenant,
  getAcaClassificationRulesDatasources,
  getAcaClassificationRulesDatasource,
}
