...
*/
const MODULE_ID = 'aca-classification-catalog-express-controller-families-delete-one';
...
const deleteOne = async (request, response) => {
  ...
  try {
    ...
    const FAMILY_ID = request?.body?.id;
    const CATALOG_ID = request?.body?.catalogId;
    params = {
      ids: [FAMILY_ID],
      catalogId: CATALOG_ID,
    };
    retVal = await familiesService.deleteManyByIds(context, params);
  } catch (error) {
