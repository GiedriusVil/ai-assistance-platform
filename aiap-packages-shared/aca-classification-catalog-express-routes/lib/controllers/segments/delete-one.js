...
*/
const MODULE_ID = 'aca-classification-catalog-express-controller-segments-delete-one';
...
const deleteOne = async (request, response) => {
  ...
  try {
    ...
    const SEGMENT_ID = request?.body?.id;
    const CATALOG_ID = request?.body?.catalogId;
    params = {
      ids: [SEGMENT_ID],
      catalogId: CATALOG_ID,
    };
    retVal = await segmentsService.deleteManyByIds(context, params);
  } catch (error) {
