...
*/
const MODULE_ID = 'aca-classification-catalog-express-controller-search-sub-classes-by-match';
...
const searchSubClassesByMatch = async (request, response) => {
  const ERRORS = [];
  let context;
  let contextUserId;
  let params;
  ...
  try {
    ...
    retVal = await subClassesService.findAllByMatch(context, params);
  } catch (error) {
  ...
  if (
    ...
    logger.error(`${searchSubClassesByMatch.name}`, { errors: ERRORS });
