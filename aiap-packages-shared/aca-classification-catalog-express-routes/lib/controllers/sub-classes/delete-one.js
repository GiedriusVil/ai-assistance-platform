...
*/
const MODULE_ID = 'aca-classification-catalog-express-controller-sub-classes-delete-one';
...
const deleteOne = async (request, response) => {
  ...
  try {
    ...
    const SUB_CLASS_ID = request?.body?.id;
    const CATALOG_ID = request?.body?.catalogId;
    params = {
      ids: [SUB_CLASS_ID],
      catalogId: CATALOG_ID,
    };
    retVal = await subClassesService.deleteManyByIds(context, params);
  } catch (error) {
