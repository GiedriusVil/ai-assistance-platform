...
*/
const MODULE_ID = 'aca-classification-catalog-express-controller-search-catalogs-by-match';
...
const searchCatalogsByMatch = async (request, response) => {
  const ERRORS = [];
  let context;
  let contextUserId;
  let params;
  ...
  try {
    ...
    retVal = await catalogsService.findAllByMatch(context, params);
  } catch (error) {
  ...
  if (
    ...
    logger.error(`${searchCatalogsByMatch.name}`, { errors: ERRORS });
