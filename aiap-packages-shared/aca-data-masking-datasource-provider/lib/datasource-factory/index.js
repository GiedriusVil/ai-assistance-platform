...
*/
const MODULE_ID = 'aca-data-masking-datasource-provider-datasource-factory';
...
const { AcaDataMaskingDatasourceMongo } = require('../datasource-mongo');
...
const _createMongoDatasource = async (configuration) => {
  try {
    ...
    const RET_VAL = new AcaDataMaskingDatasourceMongo(configuration);
...
const createAcaDataMaskingDatasource = async (configuration) => {
  try {
    ...
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error('createAcaDataMaskingDatasource', { ACA_ERROR });
    throw ACA_ERROR;
  }
}

const createAcaDataMaskingDatasources = async (configurations) => {
  try {
    if (
    ...
    const PROMISES = configurations.map((configuration) => {
      return createAcaDataMaskingDatasource(configuration);
    });
    const RET_VAL = await Promise.all(PROMISES);
    return RET_VAL;
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error('createAcaDataMaskingDatasources', { ACA_ERROR });
    throw ACA_ERROR;
  }
}

module.exports = {
  createAcaDataMaskingDatasource,
  createAcaDataMaskingDatasources,
}
