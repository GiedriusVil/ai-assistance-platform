...
*/
const MODULE_ID = 'aiap-conv-shadow-datasource-provider-index';
...
import {
  ...
  getOneById, getOneByIdAndHash,
  getRegistry
} from '@ibm-aca/aca-runtime-source-registry';

const DEFAULT_CONFIG_PATH = [Configurator.NAME];

const initManyByRootConfiguration = async (path = DEFAULT_CONFIG_PATH): Promise<void> => {
  try {
...
const initManyByTenant = async (
  ...
  try {
    ...
    const CONFIG_DATASOURCES_CONVERSATIONS = lodash.filter(CONFIG_DATASOURCES_ALL, { type: 'convShadow' });
    if (
...
const getConvShadowDatasourceByContext = (
  context: IContextV1,
): IShadowDatasourceConversationsV1 => {
  try {
    ...
    const RET_VAL = getConvShadowDatasourceByTenant(TENANT);
    return RET_VAL;
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error(getConvShadowDatasourceByContext.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

const getConvShadowDatasourceByTenant = (
  tenant: IContextTenantV1,
): IShadowDatasourceConversationsV1 => {
  try {
    ...
    if (
      ...
      const DATASOURCE_INDEX = lodash.findIndex(DATASOURCES_CONFIG, { type: 'convShadow' })
      if (
        DATASOURCE_INDEX >= 0
      ) {
        ...
        retVal = getConvShadowDatasource(DATASOURCE_ID, DATASOURCE_HASH);
      }
    }
    return retVal;
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error(getConvShadowDatasourceByTenant.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

const getConvShadowDatasource = (
  id = 'default',
  hash = undefined,
): IShadowDatasourceConversationsV1 => {
  try {
    ...
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error(getConvShadowDatasource.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

const getConvShadowDatasources = (
  context: IContextV1,
): any => {
...
export {
  ...
  initManyByTenant,
  getConvShadowDatasource,
  getConvShadowDatasources,
  getConvShadowDatasourceByContext,
