...
*/
const MODULE_ID = 'aca-audio-voice-services-express-routes-controllers-audio-voice-services-export-many';
...
const _constructFileName = (context) => {
  try {
    ...
    const RET_VAL = `[${TENANT_ID}]audio-voice-services.${currentDateAsString()}.json`;
    return RET_VAL;
  } catch (error) {
...
const exportMany = async (request, response) => {
  ...
  try {
    ...
    const DATA = await audioVoiceServicesService.exportMany(CONTEXT, PARAMS);
    if (
      lodash.isEmpty(DATA)
    ) {
      const MESSAGE = `Unable to find audio/voice services!`;
