...
*/
const MODULE_ID = 'aca-organizations-datasource-provider-datasource-factory';
...
const { AcaOrganizationsDatasourceMongo } = require('../datasource-mongo');
...
const _createMongoDatasource = async (configuration) => {
    try {
        ...
        const RET_VAL = new AcaOrganizationsDatasourceMongo(configuration);
...
const createAcaOrganizationsDatasource = async (configuration) => {
    try {
        ...
        const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
        logger.error('createAcaOrganizationsDatasource', { ACA_ERROR });
        throw ACA_ERROR;
    }
}

const createAcaOrganizationsDatasources = async (configurations) => {
    try {
        if (
        ...
        const PROMISES = configurations.map((configuration) => {
            return createAcaOrganizationsDatasource(configuration);
        });
        const RET_VAL = await Promise.all(PROMISES);
        return RET_VAL;
    } catch (error) {
        const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
        logger.error('createAcaOrganizationsDatasources', { ACA_ERROR });
        throw ACA_ERROR;
    }
}

module.exports = {
    createAcaOrganizationsDatasource,
    createAcaOrganizationsDatasources,
}
