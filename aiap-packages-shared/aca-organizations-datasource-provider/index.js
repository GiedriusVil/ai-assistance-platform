...
*/
const MODULE_ID = 'aca-organizations-datasource-provider-index';
...
const { createAcaOrganizationsDatasources } = require('./lib/datasource-factory');
...
const initManyByRootConfiguration = async (path = DEFAULT_CONFIG_PATH) => {
   ...
   if (
      ...
      const DATASOURCES = await createAcaOrganizationsDatasources(CONFIG_DATASOURCES);
...
const initManyByTenant = async (params) => {
   ...
   try {
      ...
      const CONFIG_DATASOURCES_ORGANIZATIONS = lodash.filter(CONFIG_DATASOURCES_ALL, { type: 'organizations' });
      if (
         !lodash.isEmpty(CONFIG_DATASOURCES_ORGANIZATIONS)
      ) {
         const DATASOURCES = await createAcaOrganizationsDatasources(CONFIG_DATASOURCES_ORGANIZATIONS);
...
const getAcaOrganizationsDatasourceByContext = (context) => {
   ...
   const RET_VAL = getAcaOrganizationsDatasourceByTenant(TENANT);
   return RET_VAL;
}

const getAcaOrganizationsDatasourceByTenant = (tenant) => {
   const DATASOURCES_CONFIG = ramda.path(['_datasources'], tenant);
   let retVal;
   if (
      lodash.isArray(DATASOURCES_CONFIG)
   ) {
      const DATASOURCE_INDEX = lodash.findIndex(DATASOURCES_CONFIG, { type: 'organizations' })
      if (
         DATASOURCE_INDEX >= 0
      ) {
         ...
         retVal = getAcaOrganizationsDatasource(DATASOURCE_ID, DATASOURCE_HASH);
      }
   }
   return retVal;
}

const getAcaOrganizationsDatasource = (id = 'default', hash = undefined) => {
   let retVal;
   if (
      !lodash.isEmpty(id) &&
      !lodash.isEmpty(hash)
...
const getAcaOrganizationsDatasources = (context) => {
...
module.exports = {
   ...
   initManyByTenant,
   getAcaOrganizationsDatasources,
   getAcaOrganizationsDatasourceByContext,
   getAcaOrganizationsDatasourceByTenant,
   getAcaOrganizationsDatasource,
}
