...
*/
const MODULE_ID = 'aca-engagements-service-runtime-data-synchronize-with-database';
...
const _synchronizeWithDatabaseByTenant = async (context, params) => {
  ...
  try {
    ...
    const ENGAGEMENTS = await synchronizeWithDatabaseByTenant(CONTEXT, params);
    const RET_VAL = {
    ...
    if (
      ...
      if (
        ...
        for (const RESULT of PROMISES_RESULTS) {
          logger.info(`Policy Manager Validation Engagements synchronized [TenantId: ${RESULT?.tenantId}, QTY.: ${RESULT?.engagements?.length}]`);
        }
      }
    }
  } catch (error) {
