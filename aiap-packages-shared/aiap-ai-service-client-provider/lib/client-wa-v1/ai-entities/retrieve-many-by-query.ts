...
*/
const MODULE_ID = 'aiap-ai-service-client-provider-client-wa-v1-entities-retrieve-many-by-query';
...
export const retrieveManyByQuery = async (
  ...
  try {
    ...
    response = await client.assistant.listEntities(REQUEST);
    const ITEMS = wrapIntoAiEntityV1(AI_SERVICE_TYPE_ENUM.WA_V1, response?.result?.entities)
    const RET_VAL: IRetrieveAiEntitiesByQueryResponseV1 = {
      items: ITEMS,
    }
    return RET_VAL;
  } catch (error) {
