...
*/
const MODULE_ID = `aiap-ai-service-client-provider-client-chat-gpt-v3-validate-configuration`;
...
export const _validateAiService = (
  ...
  const EXTERNAL = aiService?.external as IAiSericeExternalV1ChatGptV3;
  if (
  ...
  if (
    lodash.isEmpty(EXTERNAL?.organization)
  ) {
    const MESSAGE = `Missing required aiService?.external?.organization parameter!`;
  ...
  if (
    ...
    const MESSAGE = `Missing required aiService?.external?.apiKey parameter!`;
    const ACA_ERROR = createAcaError(MODULE_ID, ACA_ERROR_TYPE.VALIDATION_ERROR, MESSAGE);
    ERRORS.push(ACA_ERROR);
  }
  if (
