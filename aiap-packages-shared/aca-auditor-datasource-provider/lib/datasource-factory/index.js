...
*/
const MODULE_ID = 'aca-auditor-datasource-provider-datasource-factory';
...
const { AcaAuditorDatasourceMongo } = require('../datasource-mongo');
...
const _createMongoDatasource = async (configuration) => {
  try {
    ...
    const RET_VAL = new AcaAuditorDatasourceMongo(configuration);
...
const createAcaAuditorDatasource = async (configuration) => {
  try {
    ...
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error('createAcaAuditorDatasource', { ACA_ERROR });
    throw ACA_ERROR;
  }
}

const createAcaAuditorDatasources = async (configurations) => {
  try {
    if (
    ...
    const PROMISES = configurations.map((configuration) => {
      return createAcaAuditorDatasource(configuration);
    });
    const RET_VAL = await Promise.all(PROMISES);
    return RET_VAL;
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error('createAcaAuditorDatasources', { ACA_ERROR });
    throw ACA_ERROR;
  }
}

module.exports = {
  createAcaAuditorDatasource,
  createAcaAuditorDatasources,
}
