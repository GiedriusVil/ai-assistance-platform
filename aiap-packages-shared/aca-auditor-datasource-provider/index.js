...
*/
const MODULE_ID = 'aca-auditor-datasource-provider-index';
...
const { createAcaAuditorDatasources } = require('./lib/datasource-factory');
...
const initManyByRootConfiguration = async (path = DEFAULT_CONFIG_PATH) => {
  ...
  if (
    ...
    const DATASOURCES = await createAcaAuditorDatasources(CONFIG_DATASOURCES);
...
const initManyByTenant = async (params) => {
  ...
  try {
    ...
    const CONFIG_DATASOURCES_CONVERSATIONS = lodash.filter(CONFIG_DATASOURCES_ALL, { type: 'auditor' });
    if (
      !lodash.isEmpty(CONFIG_DATASOURCES_CONVERSATIONS)
    ) {
      const DATASOURCES = await createAcaAuditorDatasources(CONFIG_DATASOURCES_CONVERSATIONS);
...
const getAcaAuditorDatasourceByContext = (context) => {
  ...
  const RET_VAL = getAcaAuditorDatasourceByTenant(TENANT);
  return RET_VAL;
}

const getAcaAuditorDatasourceByTenant = (tenant) => {
  const DATASOURCES_CONFIG = ramda.path(['_datasources'], tenant);
  let retVal;
  if (
    lodash.isArray(DATASOURCES_CONFIG)
  ) {
    const DATASOURCE_INDEX = lodash.findIndex(DATASOURCES_CONFIG, { type: 'auditor' })
    if (
      DATASOURCE_INDEX >= 0
    ) {
      ...
      retVal = getAcaAuditorDatasource(DATASOURCE_ID, DATASOURCE_HASH);
    }
  }
  return retVal;
}

const getAcaAuditorDatasource = (id = 'default', hash = undefined) => {
  let retVal;
  if (
    !lodash.isEmpty(id) &&
    !lodash.isEmpty(hash)
...
const getAcaAuditorDatasources = (context) => {
...
module.exports = {
  ...
  initManyByTenant,
  getAcaAuditorDatasources,
  getAcaAuditorDatasourceByContext,
  getAcaAuditorDatasourceByTenant,
  getAcaAuditorDatasource,
}
