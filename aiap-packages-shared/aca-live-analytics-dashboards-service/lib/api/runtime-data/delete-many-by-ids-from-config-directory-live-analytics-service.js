...
*/
const MODULE_ID = 'aca-live-analytics-runtime-data-service-delete-many-from-ids-from-config-directory-dashboards-service';
...
const deleteManyByIdsFromDirectoryLiveAnalyticsDashboardsService = async (context, params) => {
  ...
  try {
    ...
    if (
      ...
      const DIR_DASHBOARDS= `${configAbsolutePath}/runtime-data-local/live-analytics/${context?.user?.session?.tenant?.id}/dashboards`;
      fsExtra.ensureDirSync(DIR_DASHBOARDS);
      ...
      if (
        ...
        for (let tmpId of params.ids) {
          if (
            ...
            fsExtra.removeSync(`${DIR_DASHBOARDS}/${tmpId}.json`);
          }
        }
      }
    }
    ...
    logger.error(deleteManyByIdsFromDirectoryLiveAnalyticsDashboardsService.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}
