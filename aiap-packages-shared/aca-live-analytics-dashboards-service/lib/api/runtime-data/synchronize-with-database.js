...
*/
const MODULE_ID = 'aca-live-analytics-dashboards-runtime-data-service-synchronize-with-database';
...
const { getAppDatasourceByContext } = require('../datasource.utils');

const { synchronizeWithDatabaseLiveAnalyticsDashboardsByTenant } = require('./synchronize-with-database-live-analytics-dashboards-by-tenant');
...
const _synchronizeWithDatabaseByTenant = async (context, params) => {
  ...
  try {
    ...
    const DASHBOARDS = await synchronizeWithDatabaseLiveAnalyticsDashboardsByTenant(CONTEXT, params);
    const RET_VAL = {
...
const synchronizeWithDatabase = async (context, params) => {
  ...
  let configLiveAnalyticsAbsolutePath;
  ...
  try {
    ...
    if (
      ...
      configLiveAnalyticsAbsolutePath = `${configAbsolutePath}/runtime-data-local/live-analytics`;
      fsExtra.ensureDirSync(configLiveAnalyticsAbsolutePath);

      const TENANT_DIRS = fsExtra.readdirSync(configLiveAnalyticsAbsolutePath);
      const PROMISES = [];
      for (let tenantId of TENANT_DIRS) {
        let params = {
          configLiveAnalyticsAbsolutePath,
          tenantId,
        };
      ...
      if (
        ...
        for (let result of PROMISES_RESULTS) {
          logger.info(`LiveAnalytics dashboards synchronized [TenantID: ${result?.tenantId}, QTY.: ${result?.value?.length}]`);
        }
      }
    }
  } catch (error) {
