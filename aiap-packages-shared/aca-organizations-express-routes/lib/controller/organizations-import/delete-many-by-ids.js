...
*/
const MODULE_ID = 'aca-organizations-express-routes-controller-organizations-import-delete-many';
...
const deleteManyByIds = async (request, response) => {
  const ERRORS = [];
  const CONTEXT_USER_ID = request?.user?.id;
  ...
  try {
    ...
    result = await organizationsImportService.deleteManyByIds(CONTEXT, PARAMS);
  } catch (error) {
