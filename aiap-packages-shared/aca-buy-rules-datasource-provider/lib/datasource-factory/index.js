...
*/
const MODULE_ID = 'aca-buy-rules-datasource-provider-datasource-factory';
...
const { AcaBuyRulesDatasourceMongo } = require('../datasource-mongo');
...
const _createMongoDatasource = async (configuration) => {
    try {
        ...
        const RET_VAL = new AcaBuyRulesDatasourceMongo(configuration);
...
const createAcaBuyRulesDatasource = async (configuration) => {
    try {
        ...
        const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
        logger.error('createAcaBuyRulesDatasource', { ACA_ERROR });
        throw ACA_ERROR;
    }
}

const createAcaBuyRulesDatasources = async (configurations) => {
    try {
        if (
        ...
        const PROMISES = configurations.map((configuration) => {
            return createAcaBuyRulesDatasource(configuration);
        });
        const RET_VAL = await Promise.all(PROMISES);
        return RET_VAL;
    } catch (error) {
        const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
        logger.error('createAcaBuyRulesDatasources', { ACA_ERROR });
        throw ACA_ERROR;
    }
}

module.exports = {
    createAcaBuyRulesDatasource,
    createAcaBuyRulesDatasources,
}
