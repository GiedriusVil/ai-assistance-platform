...
*/
const MODULE_ID = 'aca-coach-datasource-provider-datasource-factory';
...
const { AcaCoachDatasourceMongo } = require('../datasource-mongo');
...
const _createMongoDatasource = async (configuration) => {
  try {
    ...
    const RET_VAL = new AcaCoachDatasourceMongo(configuration);
...
const createAcaCoachDatasource = async (configuration) => {
  try {
    ...
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error(createAcaCoachDatasource.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

const createAcaCoachDatasources = async (configurations) => {
  try {
    if (
    ...
    const PROMISES = configurations.map((configuration) => {
      return createAcaCoachDatasource(configuration);
    });
    const RET_VAL = await Promise.all(PROMISES);
    return RET_VAL;
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error(createAcaCoachDatasources.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

module.exports = {
  createAcaCoachDatasource,
  createAcaCoachDatasources,
}
