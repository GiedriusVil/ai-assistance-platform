...
*/
const MODULE_ID = 'aca-notifications-datasource-provider-datasource-factory';
...
const { AcaNotificationsDatasourceMongo } = require('../datasource-mongo');
...
const _createMongoDatasource = async (configuration) => {
    try {
        ...
        const RET_VAL = new AcaNotificationsDatasourceMongo(configuration);
...
const createNotificationsDatasource = async (configuration) => {
    try {
        ...
        const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
        logger.error('createNotificationsDatasource', { ACA_ERROR });
        throw ACA_ERROR;
    }
}

const createNotificationsDatasources = async (configurations) => {
    try {
        if (
        ...
        const PROMISES = configurations.map((configuration) => {
            return createNotificationsDatasource(configuration);
        });
        const RET_VAL = await Promise.all(PROMISES);
        return RET_VAL;
    } catch (error) {
        const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
        logger.error('createNotificationsDatasources', { ACA_ERROR });
        throw ACA_ERROR;
    }
}

module.exports = {
    createNotificationsDatasource,
    createNotificationsDatasources,
}
