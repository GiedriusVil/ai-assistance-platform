...
*/
const MODULE_ID = 'aca-analytics-live-datasource-provider-index';
...
const { createAcaAnalyticsLiveDatasources } = require('./lib/datasource-factory');
...
const initManyByRootConfiguration = async (path = DEFAULT_CONFIG_PATH) => {
  try {
    ...
    if (
      ...
      const DATASOURCES = await createAcaAnalyticsLiveDatasources(CONFIG_DATASOURCES);
...
const initManyByTenant = async (params) => {
  try {
    ...
    const CONFIG_DATASOURCES_CONVERSATIONS = lodash.filter(CONFIG_DATASOURCES_ALL, { type: 'analyticsLive' });
    if (
      !lodash.isEmpty(CONFIG_DATASOURCES_CONVERSATIONS)
    ) {
      const DATASOURCES = await createAcaAnalyticsLiveDatasources(CONFIG_DATASOURCES_CONVERSATIONS);
...
const getAcaAnalyticsLiveDatasourceByContext = (context) => {
  try {
    ...
    const RET_VAL = getAcaAnalyticsLiveDatasourceByTenant(TENANT);
    return RET_VAL;
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error(getAcaAnalyticsLiveDatasourceByContext.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

const getAcaAnalyticsLiveDatasourceByTenant = (tenant) => {
  try {
    ...
    if (
      ...
      const DATASOURCE_INDEX = lodash.findIndex(DATASOURCES_CONFIG, { type: 'analyticsLive' })
      if (
        DATASOURCE_INDEX >= 0
      ) {
        ...
        retVal = getAcaAnalyticsLiveDatasource(DATASOURCE_ID, DATASOURCE_HASH);
      }
    }
    return retVal;
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error(getAcaAnalyticsLiveDatasourceByTenant.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

const getAcaAnalyticsLiveDatasource = (id = 'default', hash = undefined) => {
  try {
    let retVal;
    if (
      !lodash.isEmpty(id) &&
    ...
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error(getAcaAnalyticsLiveDatasource.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

const getAcaAnalyticsLiveDatasources = (context) => {
...
module.exports = {
  ...
  initManyByTenant,
  getAcaAnalyticsLiveDatasource,
  getAcaAnalyticsLiveDatasources,
  getAcaAnalyticsLiveDatasourceByContext,
  getAcaAnalyticsLiveDatasourceByTenant,
}
