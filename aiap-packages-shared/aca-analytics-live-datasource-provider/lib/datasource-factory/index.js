...
*/
const MODULE_ID = 'aca-analytics-live-datasource-provider-datasource-factory';
...
const { AcaAnalyticsLiveDatasourceMongo } = require('../datasource-mongo');
...
const _createMongoDatasource = async (configuration) => {
  try {
    ...
    const RET_VAL = new AcaAnalyticsLiveDatasourceMongo(configuration);
...
const createAcaAnalyticsLiveDatasource = async (configuration) => {
  try {
    ...
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error(createAcaAnalyticsLiveDatasource.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

const createAcaAnalyticsLiveDatasources = async (configurations) => {
  try {
    if (
    ...
    const PROMISES = configurations.map((configuration) => {
      return createAcaAnalyticsLiveDatasource(configuration);
    });
    const RET_VAL = await Promise.all(PROMISES);
    return RET_VAL;
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error(createAcaAnalyticsLiveDatasources.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

module.exports = {
  createAcaAnalyticsLiveDatasource,
  createAcaAnalyticsLiveDatasources,
}
