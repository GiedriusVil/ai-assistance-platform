...
*/
const MODULE_ID = 'aca-analytics-live-datasource-provider-utterances-find-zendesk-pop-chats-tickets-psis-tickets-by-query-month-groups';
...
const findZendeskPOPChatsTicketsPSISTicketsByQueryMonthGroups = async (datasource, context, params) => {
  const CONTEXT_USER_ID = context?.user?.id;
  const CSV_PATH = __dirname;
  const CSV_NAME = 'pop-chats-tickets-psis-tickets-data.csv';
  const CSV_DELIMITER = ',';
  const CSV_INDEX = 2;
  ...
  try {
    ...
    const RET_VAL = await getDataFromCSV(dateRangeFrom, dateRangeTo, CSV_PATH, CSV_NAME, CSV_DELIMITER, CSV_INDEX).then(function (monthsDataset) {
    ...
    logger.error(findZendeskPOPChatsTicketsPSISTicketsByQueryMonthGroups.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}
