...
*/
const MODULE_ID = 'aca-rules-datasource-provider-v2-datasource-mongo-rules-conditions-delete-many-by-rule-id';
...
const deleteManyByRuleId = async (datasource, context, params) => {
  ...
  let filter;
  try {
    const RULE_ID = params?.ruleId;
    if (
      lodash.isEmpty(RULE_ID)
    ) {
      const MESSAGE = `Missing required params.ruleId parameter!`;
