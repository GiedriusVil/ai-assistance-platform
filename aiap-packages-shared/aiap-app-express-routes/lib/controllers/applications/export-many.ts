...
*/
const MODULE_ID = 'app-express-routes-controllers-controllers-applications-export-many';
...
const _constructFileName = (
  ...
  try {
    ...
    const RET_VAL = `[${TENANT_ID}]applications.${currentDateAsString()}.json`;
    return RET_VAL;
  } catch (error) {
...
export const exportMany = async (
  ...
  let result;
  try {
    const PARAMS: IParamsV1ExportApplicationsByQuery = constructParamsV1DefaultFindManyQueryFromRequest(request);
    if (
    ...
    // }
    const DATA = await applicationsService.exportManyByQuery(CONTEXT, PARAMS);
    if (
      lodash.isEmpty(DATA)
    ) {
      const ERROR_MESSAGE = `Unable to find applications!`;
