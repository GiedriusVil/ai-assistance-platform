...
*/
const MODULE_ID = 'aca-audio-voice-services-service-datasource-utils';
...
const { getAcaAudioVoiceServicesDatasourceByContext } = require('@ibm-aca/aca-audio-voice-services-datasource-provider');
...
const getDatasourceByContext = (context) => {
  try {
    const RET_VAL = getAcaAudioVoiceServicesDatasourceByContext(context);
    if (
      lodash.isEmpty(RET_VAL)
    ) {
      const MESSAGE = 'Unable to retrieve audio voice services datasource!';
      throwAcaError(MODULE_ID, ACA_ERROR_TYPE.SYSTEM_ERROR, MESSAGE, { context: transformContextForLogger(context) });
    }
    return RET_VAL;
  } catch (error) {
