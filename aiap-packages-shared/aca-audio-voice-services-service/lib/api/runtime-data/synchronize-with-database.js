...
*/
const MODULE_ID = 'aca-audio-voice-services-runtime-data-service-synchronize-with-database';
...
const { synchronizeWithDatabaseAudioVoiceServicesByTenant } = require('./synchronize-with-database-audio-voice-service-by-tenant');
...
const _synchronizeWithDatabaseByTenant = async (context, params) => {
  ...
  try {
    ...
    const AUDIO_VOICE_SERVICES = await synchronizeWithDatabaseAudioVoiceServicesByTenant(CONTEXT, params);
    const RET_VAL = {
      tenantId: params?.tenantId,
      audioVoiceServices: AUDIO_VOICE_SERVICES
    };
    return RET_VAL;
  } catch (error) {
    ...
    if (
      ...
      if (
        ...
        for (let result of PROMISES_RESULTS) {
          logger.info(`TenantCustomizer Audio Voice Services synchronized [TenantID: ${result?.tenantId}, QTY.: ${result?.audioVoiceServices?.length}]`);
        }
      }
    }
  } catch (error) {
