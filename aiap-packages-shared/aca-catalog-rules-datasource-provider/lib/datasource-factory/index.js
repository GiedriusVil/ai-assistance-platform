...
*/
const MODULE_ID = 'aca-catalog-rules-datasource-provider-datasource-factory';
...
const { AcaCatalogRulesDatasourceMongo } = require('../datasource-mongo');
...
const _createMongoDatasource = async (configuration) => {
    try {
        ...
        const RET_VAL = new AcaCatalogRulesDatasourceMongo(configuration);
...
const createAcaCatalogRulesDatasource = async (configuration) => {
    try {
        const RET_VAL = await _createMongoDatasource(configuration);
        return RET_VAL;
    } catch (error) {
        const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
        logger.error('createAcaCatalogRulesDatasource', { ACA_ERROR });
        throw ACA_ERROR;
    }
}

const createAcaCatalogRulesDatasources = async (configurations) => {
    try {
        if (
        ...
        const PROMISES = configurations.map((configuration) => {
            return createAcaCatalogRulesDatasource(configuration);
        });
        const RET_VAL = await Promise.all(PROMISES);
        return RET_VAL;
    } catch (error) {
        const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
        logger.error('createAcaCatalogRulesDatasources', { ACA_ERROR });
        throw ACA_ERROR;
    }
}

module.exports = {
    createAcaCatalogRulesDatasource,
    createAcaCatalogRulesDatasources,
}
