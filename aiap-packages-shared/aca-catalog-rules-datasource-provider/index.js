...
*/
const MODULE_ID = 'aca-catalog-rules-datasource-provider-index';
...
const { createAcaCatalogRulesDatasources } = require('./lib/datasource-factory');
...
const initManyByRootConfiguration = async (path = DEFAULT_CONFIG_PATH) => {
  ...
  if (
    ...
    const DATASOURCES = await createAcaCatalogRulesDatasources(CONFIG_DATASOURCES);
...
const initManyByTenant = async (params) => {
  ...
  try {
    ...
    const CONFIG_DATASOURCES_CONVERSATIONS = lodash.filter(CONFIG_DATASOURCES_ALL, { type: 'catalogRules' });
    if (
      !lodash.isEmpty(CONFIG_DATASOURCES_CONVERSATIONS)
    ) {
      
      const DATASOURCES = await createAcaCatalogRulesDatasources(CONFIG_DATASOURCES_CONVERSATIONS);
...
const getAcaCatalogRulesDatasourceByContext = (context) => {
  ...
  const RET_VAL = getAcaCatalogRulesDatasourceByTenant(TENANT);
  return RET_VAL;
}

const getAcaCatalogRulesDatasourceByTenant = (tenant) => {
  const DATASOURCES_CONFIG = ramda.path(['_datasources'], tenant);
  let retVal;
  if (
    lodash.isArray(DATASOURCES_CONFIG)
  ) {
    const DATASOURCE_INDEX = lodash.findIndex(DATASOURCES_CONFIG, { type: 'catalogRules' })
    if (
      DATASOURCE_INDEX >= 0
    ) {
      ...
      retVal = getAcaCatalogRulesDatasource(DATASOURCE_ID, DATASOURCE_HASH);
    }
  }
  return retVal;
}

const getAcaCatalogRulesDatasource = (id = 'default', hash = undefined) => {
  let retVal;
  if (
    !lodash.isEmpty(id) &&
    !lodash.isEmpty(hash)
...
const getAcaCatalogRulesDatasources = (context) => {
...
module.exports = {
  ...
  initManyByTenant,
  getAcaCatalogRulesDatasourceByContext,
  getAcaCatalogRulesDatasourceByTenant,
  getAcaCatalogRulesDatasources,
  getAcaCatalogRulesDatasource,
}
