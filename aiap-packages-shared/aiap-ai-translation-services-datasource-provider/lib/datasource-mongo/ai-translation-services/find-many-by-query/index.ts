...
*/
const MODULE_ID = 'aiap-ai-translation-services-datasource-mongo-ai-translation-services-find-many-by-query';
...
const findManyByQuery = async (
  ...
  context: IContextV1,
  params: IAiTranslationServicesFindManyByQueryParamsV1,
  ...
  try {
    ...
    const AI_TRANSLATION_SERVICES = ramda.pathOr([], ['items'], RESULT);
    const TOTAL = ramda.pathOr(AI_TRANSLATION_SERVICES.length, ['total'], RESULT);

    const RET_VAL: IAiTranslationServicesFindManyByQueryResponseV1 = {
      items: formatResponse(datasource.configuration, AI_TRANSLATION_SERVICES),
      total: TOTAL
    };
    return RET_VAL;
  } catch (error) {
