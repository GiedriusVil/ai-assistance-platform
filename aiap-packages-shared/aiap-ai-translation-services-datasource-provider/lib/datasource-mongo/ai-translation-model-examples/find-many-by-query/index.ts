...
*/
const MODULE_ID = 'aiap-ai-translation-services-datasource-mongo-ai-translation-model-examples-find-many-by-query';
...
const findManyByQuery = async (
  ...
  context: IContextV1,
  params: IAiTranslationModelExamplesFindManyByQueryParamsV1,
  ...
  try {
    ...
    const AI_TRANSLATION_MODEL_EXAMPES = ramda.pathOr([], ['items'], RESULT);
    const TOTAL = ramda.pathOr(AI_TRANSLATION_MODEL_EXAMPES.length, ['total'], RESULT);

    const RET_VAL: IAiTranslationModelExamplesFindManyByQueryResponseV1 = {
      items: formatResponse(AI_TRANSLATION_MODEL_EXAMPES),
      total: TOTAL
    };
    return RET_VAL;
  } catch (error) {
