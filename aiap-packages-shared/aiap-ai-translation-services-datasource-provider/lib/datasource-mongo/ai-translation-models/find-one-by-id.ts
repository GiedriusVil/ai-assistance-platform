...
*/
const MODULE_ID = 'aiap-ai-translation-services-datasource-mongo-ai-translation-models-find-one-by-id';
...
const findOneById = async (
  ...
  context: IContextV1,
  params: IAiTranslationModelsFindOneByIdParamsV1,
): Promise<IAiTranslationModelsFindOneByIdResponseV1> => {
  const CONTEXT_USER_ID = context?.user?.id;
  const AI_TRANSLATION_MODEL_ID = params?.id;
  ...
  try {
    ...
    const RET_VAL: IAiTranslationModelsFindOneByIdResponseV1 = ramda.pathOr({}, [0], RESPONSE);
