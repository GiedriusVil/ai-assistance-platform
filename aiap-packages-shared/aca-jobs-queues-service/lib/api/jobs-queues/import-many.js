...
*/
const MODULE_ID = 'aca-jobs-queues-service-jobs-queues-import-many';
...
const importMany = async (context, params) => {
  try {
    ...
    if (
      ...
      const MESSAGE = 'Missing jobs queues in file!';
    ...
    if (
      ...
      const MESSAGE = `Jobs queues are not compatible for import! File must contain 'id'!`;
      throwAcaError(MODULE_ID, ACA_ERROR_TYPE.VALIDATION_ERROR, MESSAGE);
    }
    const PROMISES = [];
    for (let jobsQueues of RECORDS_FROM_FILE) {
      PROMISES.push(saveOne(context, { jobsQueues }));
    }
