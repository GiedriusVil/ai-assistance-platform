...
*/
const MODULE_ID = 'aca-classification-catalog-service-datasource-utils';
...
const { getAcaClassificationCatalogDatasourceByContext } = require('@ibm-aca/aca-classification-catalog-datasource-provider');
const { getDatasourceV1App } = require('@ibm-aiap/aiap-app-datasource-provider');

const getDatasourceByContext = (context) => {
  try {
    const RET_VAL = getAcaClassificationCatalogDatasourceByContext(context);
    if (
