...
*/
const MODULE_ID = 'aca-classification-catalog-service-runtime-data-service-synchronize-with-config-directory-sub-class';
...
const synchronizeWithConfigDirectorySubClass = async (context, params) => {
  ...
  try {
    ...
    if (
      ...
      if (
        lodash.isEmpty(params?.subClass?.id)
      ) {
      ...
      let filePath = `${configAbsolutePath}/runtime-data-local/tenant-customizer/${context?.user?.session?.tenant?.id}/classification-catalogs/sub-classes/${params?.subClass?.id}.json`;
      fsExtra.ensureDirSync(`${configAbsolutePath}/runtime-data-local/tenant-customizer/${context?.user?.session?.tenant?.id}/classification-catalogs/sub-classes`);
      fsExtra.writeFileSync(
        filePath,
        JSON.stringify(params?.subClass, null, 4)
      );
    }
  } catch (error) {
    ...
    logger.error(synchronizeWithConfigDirectorySubClass.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}
