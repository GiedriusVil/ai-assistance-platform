...
*/
const MODULE_ID = 'aca-classification-catalog-service-runtime-data-service-synchronize-with-config-directory-family';
...
const synchronizeWithConfigDirectoryFamily = async (context, params) => {
  ...
  try {
    ...
    if (
      ...
      if (
        lodash.isEmpty(params?.family?.id)
      ) {
      ...
      let filePath = `${configAbsolutePath}/runtime-data-local/tenant-customizer/${context?.user?.session?.tenant?.id}/classification-catalogs/families/${params?.family?.id}.json`;
      fsExtra.ensureDirSync(`${configAbsolutePath}/runtime-data-local/tenant-customizer/${context?.user?.session?.tenant?.id}/classification-catalogs/families`);
      fsExtra.writeFileSync(
        filePath,
        JSON.stringify(params?.family, null, 4)
      );
    }
  } catch (error) {
    ...
    logger.error(synchronizeWithConfigDirectoryFamily.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}
