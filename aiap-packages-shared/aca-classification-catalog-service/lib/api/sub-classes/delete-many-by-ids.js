...
*/
const MODULE_ID = 'aca-classification-catalog-service-sub-classes-delete-many-by-ids';
...
const deleteManyByIds = async (context, params) => {
    ...
    try {
        ...
        const RET_VAL = await DATASOURCE.subClasses.deleteManyByIds(context, params);
        const PROMISES = [];
        if (
            ...
            for (let id of PARAMS_IDS) {
                let action = {
                    ...
                    type: ACTION_TYPES.SUB_CLASS_DELETE,
                }
                PROMISES.push(saveAction(context, { action: action }))
            }
        }
