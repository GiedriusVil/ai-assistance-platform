...
*/
const MODULE_ID = 'aca-classification-catalog-service-catalogs-import-many';
...
const importMany = async (context, params) => {
  try {
    ...
    if (
      ...
      const MESSAGE = 'Missing classification catalogs in file!';
    ...
    if (
      ...
      const MESSAGE = `Classification catalogs are not compatible for import! File must contain 'id'!`;
      throwAcaError(MODULE_ID, ACA_ERROR_TYPE.VALIDATION_ERROR, MESSAGE);
    }
    const PROMISES = [];
    for (let catalog of RECORDS_FROM_FILE) {
      PROMISES.push(saveOne(context, { catalog }));
    }
