...
*/
const MODULE_ID = 'app-datasource-mongo-tenants-find-one-by-id-and-hash';
...
export const findOneByIdAndHash = async (
  ...
  try {
    if (
      ...
      const MESSAGE = 'Required params.id paramter is missing!';
      throwAcaError(MODULE_ID, ACA_ERROR_TYPE.VALIDATION_ERROR, MESSAGE);
    }
    if (
      lodash.isEmpty(params?.hash)
    ) {
      const MESSAGE = 'Required params.hash parameter is missing!';
      throwAcaError(MODULE_ID, ACA_ERROR_TYPE.VALIDATION_ERROR, MESSAGE);
    }
    filter = {
      _id: {
