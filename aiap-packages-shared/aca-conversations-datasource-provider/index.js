...
*/
const MODULE_ID = 'aca-conversations-datasource-provider-index';
...
const { createAcaConversationsDatasources } = require('./lib/datasource-factory');
...
const initManyByRootConfiguration = async (path = DEFAULT_CONFIG_PATH) => {
  try {
    ...
    if (
      ...
      const DATASOURCES = await createAcaConversationsDatasources(CONFIG_DATASOURCES);
...
const initManyByTenant = async (params) => {
  try {
    ...
    const CONFIG_DATASOURCES_CONVERSATIONS = lodash.filter(CONFIG_DATASOURCES_ALL, { type: 'conversations' });
    if (
      !lodash.isEmpty(CONFIG_DATASOURCES_CONVERSATIONS)
    ) {
      const DATASOURCES = await createAcaConversationsDatasources(CONFIG_DATASOURCES_CONVERSATIONS);
...
const getAcaConversationsDatasourceByContext = (context) => {
  try {
    ...
    const RET_VAL = getAcaConversationsDatasourceByTenant(TENANT);
    return RET_VAL;
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error(getAcaConversationsDatasourceByContext.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

const getAcaConversationsDatasourceByTenant = (tenant) => {
  try {
    ...
    if (
      ...
      const DATASOURCE_INDEX = lodash.findIndex(DATASOURCES_CONFIG, { type: 'conversations' })
      if (
        DATASOURCE_INDEX >= 0
      ) {
        ...
        retVal = getAcaConversationsDatasource(DATASOURCE_ID, DATASOURCE_HASH);
      }
    }
    return retVal;
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error(getAcaConversationsDatasourceByTenant.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

const getAcaConversationsDatasource = (id = 'default', hash = undefined) => {
  try {
    let retVal;
    if (
      !lodash.isEmpty(id) &&
    ...
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error(getAcaConversationsDatasource.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

const getAcaConversationsDatasources = (context) => {
...
module.exports = {
  ...
  initManyByTenant,
  getAcaConversationsDatasource,
  getAcaConversationsDatasources,
