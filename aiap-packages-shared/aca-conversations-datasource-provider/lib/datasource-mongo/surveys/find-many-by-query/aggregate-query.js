/*
  © Copyright IBM Corporation 2022. All Rights Reserved 
   
  SPDX-License-Identifier: EPL-2.0
*/
const {
  matchFieldBetween2Dates,
  matchAttributeConversationId,
  matchAttributeAssistantIdByParams,
  matchAttributeAssistantIdByContext,
  addSortCondition,
  addPagination,
  matchAttributeByRegex,
} = require('@ibm-aiap/aiap-utils-mongo');

const surveysMattcher = (context, params) => {
  const FILTERS = params?.filter;
  const RET_VAL = {
    $match: {
      $and: [
        matchFieldBetween2Dates('created', FILTERS),
        matchAttributeAssistantIdByContext(context),
        matchAttributeAssistantIdByParams(FILTERS),
        matchAttributeConversationId(FILTERS),
        {
          $or: [
            matchAttributeByRegex('_id', FILTERS?.search),
            matchAttributeByRegex('conversationId', FILTERS?.search),
          ]
        }
      ]
    },
  };
  return RET_VAL;
}

const aggregateQuery = (context, params) => {
  const RET_VAL = [
    surveysMattcher(context, params),
    {
      $facet: {
        items: [
          ...addSortCondition(params),
          ...addPagination(params),
        ],
        total: [
          {
            $count: 'count',
          }
        ]
      }
    }
  ];
  RET_VAL.push(
    {
      $set: { tempTotal: { $arrayElemAt: ['$total', 0] } }
    }
  );
  RET_VAL.push(
    {
      $project: {
        items: 1,
        total: { $ifNull: ['$tempTotal.count', 0] },
      }
    }
  );
  return RET_VAL;
}

module.exports = {
  aggregateQuery,
}
