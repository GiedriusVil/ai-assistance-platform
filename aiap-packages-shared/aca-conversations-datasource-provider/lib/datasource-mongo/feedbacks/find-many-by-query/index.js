...
*/
const MODULE_ID = 'aca-conversations-datasoure-feedbacks-find-many-by-query';
...
const findManyByQuery = async (datasource, context, params) => {
  ...
  try {
    ...
    const FEEDBACKS = ramda.pathOr([], ['feedbacks'], RESULT);
    const TOTAL = ramda.pathOr({}, ['total'], RESULT);
    const RET_VAL = {
      items: formatResponse(FEEDBACKS),
      total: ramda.pathOr(FEEDBACKS.length, [0, 'count'], TOTAL)
    };
    return RET_VAL;
  } catch (error) {
