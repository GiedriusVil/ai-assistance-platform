...
const _synchronizeWithDatabaseByTenant = async (context, params) => {
  ...
  try {
    ...
    const ORGANIZATIONS = await synchronizeWithDatabaseByTenant(CONTEXT, params);
    const RET_VAL = {
    ...
    if (
      ...
      if (
        ...
        for (const RESULT of PROMISES_RESULTS) {
          logger.info(`Policy Manager Organizations synchronized [TenantId: ${RESULT?.tenantId}, QTY.: ${RESULT?.organizations?.length}]`);
        }
      }
    }
  } catch (error) {
