...
*/
const MODULE_ID = 'aca-organizations-service-runtime-data-delete-many-by-ids-from-config-directory';
...
const deleteManyByIdsFromConfigDirectory = async (context, params) => {
  ...
  try {
    ...
    if (
      ...
      const DIR_ORGANIZATIONS = `${configAbsolutePath}/runtime-data-local/policy-manager/${context?.user?.session?.tenant?.id}/organizations`;
      fsExtra.ensureDirSync(DIR_ORGANIZATIONS);
      if (
