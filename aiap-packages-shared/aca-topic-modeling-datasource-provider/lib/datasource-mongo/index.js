...
*/
const MODULE_ID = 'aca-topic-modeling-datasource-provider-datasource-mongo';
...
const { AcaTopicModelingDatasource } = require('../datasource');
const { getAcaMongoClient } = require('@ibm-aiap/aiap-mongo-client-provider');
const { createIndex } = require('@ibm-aiap/aiap-utils-mongo')

const _topicModels = require('./topic-models');
const { sanitizedCollectionsFromConfiguration } = require('./collections.utils');

class AcaTopicModelingDatasourceMongo extends AcaTopicModelingDatasource {
  ...
  async _ensureIndexes() {
    try {
      ...
      await createIndex(DB, COLLECTIONS.topicModels, { modelId: 1 });
    } catch (error) {
      const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
  ...
  get topicModels() {
    const RET_VAL = {
      findManyByQuery: async (context, params) => {
        return _topicModels.findManyByQuery(this, context, params);
      },
      findOneById: async (context, params) => {
        return _topicModels.findOneById(this, context, params);
      },
      saveOne: async (context, params) => {
        return _topicModels.saveOne(this, context, params);
      },
      deleteManyByIds: async (context, params) => {
        return _topicModels.deleteManyByIds(this, context, params);
      },
    };
    return RET_VAL;
  }
...
module.exports = {
  AcaTopicModelingDatasourceMongo
};
