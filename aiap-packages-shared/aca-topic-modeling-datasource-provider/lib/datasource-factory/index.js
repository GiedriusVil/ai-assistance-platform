...
*/
const MODULE_ID = 'aca-topic-modeling-datasource-provider-datasource-factory';
...
const { AcaTopicModelingDatasourceMongo } = require('../datasource-mongo');
...
const _createMongoDatasource = async (configuration) => {
  try {
    ...
    const RET_VAL = new AcaTopicModelingDatasourceMongo(configuration);
...
const createAcaTopicModelingDatasource = async (configuration) => {
  try {
    ...
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error(createAcaTopicModelingDatasource.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

const createAcaTopicModelingDatasources = async (configurations) => {
  try {
    if (
    ...
    const PROMISES = configurations.map((configuration) => {
      return createAcaTopicModelingDatasource(configuration);
    });
    const RET_VAL = await Promise.all(PROMISES);
    return RET_VAL;
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    logger.error(createAcaTopicModelingDatasources.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

module.exports = {
  createAcaTopicModelingDatasource,
  createAcaTopicModelingDatasources,
}
