...
*/
const MODULE_ID = 'aiap-ai-translation-services-express-routes-controllers-ai-translation-services-test-one-by-service-model-id';
...
const testByServiceModelId = async (
  ...
  try {
    ...
    result = await aiTranslationModelsService.translateTextByServiceModelId(CONTEXT, PARAMS);
  } catch (error) {
  ...
  if (
    ...
    logger.error(testByServiceModelId.name, { errors: ERRORS });
    response.status(500).json({ errors: ERRORS });
  }
};

export {
  testByServiceModelId,
};
