...
*/
const MODULE_ID = 'aiap-ai-translation-service-express-routes-controllers-ai-translation-prompts-export-many-by-query';
...
const _constructFileName = (
  ...
  try {
    ...
    const RET_VAL = `[${TENANT_ID}]ai-translation-prompts.${currentDateAsString()}.json`;
    return RET_VAL;
  } catch (error) {
...
const exportManyByQuery = async (
  ...
  let result;
  try {
    const RESULT = await aiTranslationPromptsService.exportMany(CONTEXT, PARAMS?.query);
    if (
      lodash.isEmpty(RESULT)
    ) {
      const MESSAGE = `Unable to find ai-translation-prompts!`;
