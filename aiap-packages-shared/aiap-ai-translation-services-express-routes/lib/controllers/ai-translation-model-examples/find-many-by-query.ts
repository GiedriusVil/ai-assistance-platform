...
*/
const MODULE_ID = 'aiap-ai-translation-service-express-routes-controllers-ai-translation-model-examples-find-many-by-query';
...
const findManyByQuery = async (
  ...
  try {
    ...
    const AI_TRANSLATION_MODEL_ID = QUERY?.filter?.aiTranslationModelId;
    if (
    ...
    if (
      ...
      const MESSAGE = `Missing required request.body.query.filter.aiTranslationModelId paramater!`;
      throwAcaError(MODULE_ID, ACA_ERROR_TYPE.VALIDATION_ERROR, MESSAGE);
    }
    const PARAMS = {
      ...QUERY
    };
    result = await aiTranslationModelExamplesService.findManyByQuery(CONTEXT, PARAMS);
  } catch (error) {
