...
*/
const MODULE_ID = 'aca-rules-engine-provider-json-rule-engine-operator-total-item-amount-less-than';
...
const _operator = (factValue, conditionValue) => {
  try {
    ...
    if (
      ...
      const BASE = factValue?.price?.base?.value;
      const PRICE = factValue?.price?.money?.value;
      const QUANTITY = factValue?.quantity?.value;
      const TOTAL_ITEM_AMOUNT = Number(QUANTITY / BASE * PRICE).toFixed(6);
      const JSON_VALUE_INTEGER = Number(conditionValue);
      retVal = TOTAL_ITEM_AMOUNT < JSON_VALUE_INTEGER;
    }
    return retVal;
  } catch (error) {
...
const addOperatorTotalItemAmountLessThan = (operators) => {
  operators.push(new Operator('totalItemAmountLessThan', VALIDATOR));
}


module.exports = {
  addOperatorTotalItemAmountLessThan,
}
