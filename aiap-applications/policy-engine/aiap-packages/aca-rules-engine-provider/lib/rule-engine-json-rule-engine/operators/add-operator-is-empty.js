...
*/
const MODULE_ID = 'aca-rules-engine-provider-json-rule-engine-operators-add-operator-is-empty';
...
const _operator = (factValue, conditionValue) => {
  try {
    const RET_VAL = lodash.isEmpty(factValue);
    return RET_VAL;
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    appendDataToError(ACA_ERROR, { conditionValue });
...
const addOperatorIsEmpty = (operators) => {
  operators.push(new Operator('isEmpty', VALIDATOR));
}
