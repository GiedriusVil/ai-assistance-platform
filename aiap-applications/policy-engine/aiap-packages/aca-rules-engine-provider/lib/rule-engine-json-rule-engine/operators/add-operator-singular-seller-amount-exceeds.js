...
*/
const MODULE_ID = 'aca-rules-engine-provider-json-rule-engine-operators-add-operator-singular-seller-amount-exceeds';
...
const _operator = (factValue, conditionValue) => {
  try {
    ...
    if (
      ...
      const SELLERS = factValue.filter(seller => seller.totalAmount > conditionValue);
...
const addOperatorSingularSellerAmountExceeds = (operators) => {
  operators.push(new Operator('singularSellerAmountExceeds', VALIDATOR));
}

module.exports = {
  addOperatorSingularSellerAmountExceeds,
}
