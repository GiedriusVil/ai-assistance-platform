...
*/
const MODULE_ID = `aca-json-rule-engine-operators-default-add-operator-does-not-contain`;
...
const _operator = (factValue, conditionValue) => {
  const RET_VAL = factValue?.indexOf(conditionValue) === -1
  return RET_VAL;
}
...
const addOperatorDoesNotContain = (operators) => {
  operators.push(new Operator('doesNotContain', VALIDATOR));
}

module.exports = {
  addOperatorDoesNotContain,
}
