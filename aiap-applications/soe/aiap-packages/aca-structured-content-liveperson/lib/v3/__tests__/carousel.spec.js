...
describe('carousel structured message', () => {
  ...
  const transformedEmptyTemplate = {
    type: 'carousel',
    padding: 10,
    elements: [],
  };
  ...
  test('should construct structured message - button has link', () => {
    ...
    const structuredMessage = carousel(carouselDataWithLinks.attachments);

    expect(structuredMessage).toMatchObject(transformedCarouselWithLinks);
  });

  test('should construct structured message - button has payload', () => {
    const carousel = require('../carousel');
    const structuredMessage = carousel(carouselDataWithPayload.attachments);

    expect(structuredMessage).toMatchObject(transformedCarouselWithPayload);
  });

  test('should return undefined if attachment array is empty', () => {
    const carousel = require('../carousel');
    const structuredMessage = carousel([]);

    expect(structuredMessage).toMatchObject(transformedEmptyTemplate);
  });

  test('should return undefined if attachment array not defined', () => {
    const carousel = require('../carousel');
    const structuredMessage = carousel();
