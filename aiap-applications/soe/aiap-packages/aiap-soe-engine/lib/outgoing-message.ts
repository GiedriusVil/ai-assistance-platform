...
import {
  ...
  set,
  unset,
} from '@ibm-aca/aca-wrapper-lodash';

import uuid from 'uuid/v4';

import {
  ISoeMessageAttachmentV1,
  ISoeLeftPanelV1,
...
export class OutgoingMessage {
   ...
   * @return {OutgoinMessage} returns this object to allow for chaining of methods.
   */
  addRecipientByPhoneNumber(phoneNumber: string) {
    const recipient = {
   ...
   */
  addAttachment(
    attachment: ISoeMessageAttachmentV1,
  ) {
   ...
   * @return {OutgoinMessage} returns this object to allow for chaining of methods.
   */
  addAttachmentFromUrl(type: string, url: string) {
    if (!type || !url) {
   ...
   * @return {OutgoinMessage} returns this object to allow for chaining of methods.
   */
  addQuickReplies(quickReplies: any) {
   ...
   * @return {OutgoinMessage} returns this object to allow for chaining of methods.
   */
  addPayloadLessQuickReplies(quickRepliesTitles: any[]) {
   ...
   * @return {OutgoinMessage} returns this object to allow for chaining of methods.
   */
  addSenderAction(senderAction: string) {
  ...
  /**
   * Adds `message.leftPanel` to the OutgoingMessage.
   *
   * @param {object} leftPanel valid left panel type object
   * @return {OutgoinMessage} returns this object to allow for chaining of methods.
   */
  addLeftPanelChanges(
    leftPanel: ISoeLeftPanelV1,
  ) {
    return this.__addProperty('message.leftPanel', 'leftPanel', leftPanel);
  }
}
