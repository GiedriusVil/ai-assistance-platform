...
class LastStateWare extends AbstractMiddleware {
  ...
  executor(adapter, update) {
    ...
    try {
      ...
      if (
        ...
        if (logger.isDebug())
          logger.debug(
            'AiService response does not have retryTag/fail - saving state',
            { update }
          );
        update.session.state = update.session.lastState;
      } else {
        if (logger.isDebug())
          logger.debug('Contains retry/fail tag, not saving state!', {
            update,
          });
      }
      return;
