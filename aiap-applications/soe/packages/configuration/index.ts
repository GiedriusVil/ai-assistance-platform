...
const { transformRawConfiguration } = require('./lib/transformer');

const { isStringTrue } = require('./lib/utils');
...
const loadConfiguration = async () => {
  ...
  const RET_VAL = await mergeConfiguration(TRANSFORMED_CONFIG, schema);
...
const getConfiguration = () => {
  if (configuration) {
    ...
    throw new Error('Please load configuration first!');
  }
};
