...
*/
const MODULE_ID = 'release-assistant-2023-04-17-rules-validations-process-operations';
...
const processOperations = async (config, operations, db) => {
  try {
    ...
    for (let bucket of BUCKETS) {
      PROMISES.push(db.collection(config.app.collections.rules).bulkWrite(bucket, { ordered: false }));
    }
