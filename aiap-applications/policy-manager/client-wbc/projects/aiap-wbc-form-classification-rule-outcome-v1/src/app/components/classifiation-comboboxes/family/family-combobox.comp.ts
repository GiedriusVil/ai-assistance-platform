...
@Component({
  selector: 'aca-external-classification-family-combobox',
  templateUrl: './family-combobox.comp.html',
  styleUrls: ['./family-combobox.comp.scss'],
  viewProviders: [{ provide: ControlContainer, useExisting: NgForm }]
})
export class ExternalClassificationFamilyCombobox extends BaseCombobox implements OnInit, OnChanges {

  static getClassName() {
    return 'ExternalClassificationFamilyCombobox';
  }
  ...
  _queryState: any = {
    ...
    pageSize: 10,
    page: 1,
    queryType: EXTERNAL_QUERIES.CLASSIFICATION_RULES_FAMILY.TYPE,
  }
  ...
  ngOnChanges(changes: SimpleChanges): void {
    _debugX(ExternalClassificationFamilyCombobox.getClassName(), 'ngOnChanges', {
      changes: changes,
    ...
    this.handleHigherClassificationValueChange(changes?.segment, QueryServiceV1.FILTER_KEY.SEGMENT_ID, changes?.value, true);
  }
  ...
  protected loadItems() {
    ...
    _debugX(ExternalClassificationFamilyCombobox.getClassName(), 'loadItems', { QUERY });
    this.eventsService.loadingEmit(true);
    this.queryState.loading = true;
    this.classificationRulesClassificationsExternalService.findManyFamiliesByQuery(QUERY)
      .pipe(
        catchError(error => this.handleFindManyByQueryError(error)),
        takeUntil(this._destroyed$),
      ).subscribe((response: any) => {
        _debugX(ExternalClassificationFamilyCombobox.getClassName(), 'loadItems', { response });
  ...
  private handleFindManyByQueryError(error: any) {
    _debugX(ExternalClassificationFamilyCombobox.getClassName(), 'handleSubmitEvent', { error });
    const NOTIFICATION = CLASSIFICATION_RULES_EXTERNAL_CLASSIFICATIONS_MESSAGES.ERROR.FIND_MANY_FAMILIES_BY_QUERY;
