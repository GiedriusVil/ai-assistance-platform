...
@Injectable()
export class DocValidationMetricsServiceV1 extends BaseServiceV1 {

  static getClassName() {
    return 'DocValidationMetricsServiceV1';
  }
  ...
  findManyByQuery(query: any) {
    ...
    _debugX(DocValidationMetricsServiceV1.getClassName(), 'findManyByQuery',
      {
        QUERY
      });
  ...
  retriveMetrics(query: any) {
    const QUERY = lodash.cloneDeep(query);
    _debugX(DocValidationMetricsServiceV1.getClassName(), 'retrieveMetrics',
      {
        QUERY
      });
  ...
  retrievePRCountByDay(query: any) {
    const QUERY = lodash.cloneDeep(query);
    _debugX(DocValidationMetricsServiceV1.getClassName(), 'retrievePRCountByDay',
      {
        QUERY
      });
  ...
  retriveValidationFrequency(query: any) {
    const QUERY = lodash.cloneDeep(query);
    _debugX(DocValidationMetricsServiceV1.getClassName(), 'retriveValidationFrequency',
      {
        QUERY
      });
