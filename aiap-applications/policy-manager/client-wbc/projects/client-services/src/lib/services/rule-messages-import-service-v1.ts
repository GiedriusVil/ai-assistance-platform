...
@Injectable()
export class RuleMessagesImportServiceV1 extends BaseServiceV1 {

  static getClassName(): string {
    return 'RuleMessagesImportServiceV1';
  }
  ...
  _hostAndBasePath() {
    let retVal = `${this._hostUrl()}/api/import/rules-messages`;
    return retVal;
  }
  ...
  uploadFile(file: File) {
    ...
    formData.append('messagesFile', file, file.name);
    const REQUEST_OPTIONS = {
      headers: { ['Authorization']: `Bearer ${this._token()}` }
    };
