...
export class RuleOutcomeFormV2 extends BaseFieldWbcV1 implements OnInit, OnDestroy, OnChanges {
  ...
  handleActionsValueChange(event: any) {
    _debugW(RuleOutcomeFormV2.getClassName(), 'handleActionsValueChange',
      {
        event: event,
        this_value: this.value,
        this_state: this.state,
    ...
    VALUE_NEW.actions = event;
    this.valueChange.emit(VALUE_NEW);
  }

  handleMessageValueChange(event: any) {
    const VALUE_NEW = lodash.cloneDeep(this.value);
    VALUE_NEW.message = event;
    _debugW(RuleOutcomeFormV2.getClassName(), 'handleMessageValueChange',
      {
        event: event,
        this_value: this.value,
        this_state: this.state,
