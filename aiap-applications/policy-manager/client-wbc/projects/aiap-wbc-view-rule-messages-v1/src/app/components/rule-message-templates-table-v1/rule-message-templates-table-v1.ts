...
export class RuleMessageTemplatesTableV1 implements OnInit, OnDestroy, OnChanges, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.ruleMessagesService.retrieveMessageSaveFormData()
      .pipe(
        tap(() => this.eventsService.loadingEmit(true)),
        catchError((error) => this.handleRetrieveMessageMessageTemplatesTableDataError(error)),
        takeUntil(this._destroyed$),
      ).subscribe((response: any) => {
        _debugW(RuleMessageTemplatesTableV1.getClassName(), 'ngOnInit',
          {
            response,
          });
  ...
  handleRetrieveMessageMessageTemplatesTableDataError(error: any) {
    ...
    const NOTIFICATION = {
      type: "error",
      title: this.translateService.instant('view_rule_messages_v1.templates_table_v1.notification.error'),
      message: message,
  ...
  constructTableHeader() {
    ...
    TABLE_HEADER.push(new TableHeaderItem({
      data: this.translateService.instant('view_rule_messages_v1.templates_table_v1.message.header'),
      field: 'message',
      style: { width: '70%' }
    }));
    TABLE_HEADER.push(new TableHeaderItem({
      data: this.translateService.instant('view_rule_messages_v1.templates_table_v1.language.header'),
      field: 'language',
      style: { width: '20%' }
    }));
    TABLE_HEADER.push(new TableHeaderItem({
      data: this.translateService.instant('view_rule_messages_v1.templates_table_v1.action.header'),
      style: { width: '10%' }
    }));
  ...
  setLanguages(template: any) {
    ...
    } else if (!lodash.isEmpty(languages)) {
  ...
  refreshTable() {
    ...
    if (
      ...
      MESSAGE_TEMPLATES.forEach((template, index) => {
        ...
        _debugW(RuleMessageTemplatesTableV1.getClassName(), 'refreshTable',
          {
            index,
  ...
  handleLanguagesSelect(event: any, template: any) {
    _debugW(RuleMessageTemplatesTableV1.getClassName(), 'handleLanguagesSelect', {
      template,
      event,
    ...
    const TEMPLATE_LANGUAGE = event?.item?.code;
  ...
  _removeTemplateFromTableModel(index: any) {
    ...
    _debugW(RuleMessageTemplatesTableV1.getClassName(), '_removeTemplateFromTableModel', {
      index,
      arrayIndex
    });
  ...
  _removeTemplateFromMessageTemplates(index: any) {
    _debugW(RuleMessageTemplatesTableV1.getClassName(), '_removeTemplateFromMessageTemplates', {
      index
    });
  ...
  _addTemplateToTableModel(template: any) {
    _debugW(RuleMessageTemplatesTableV1.getClassName(), '_addTemplateToTableModel', {
      template
    });
  ...
  _addTemplate(template: any) {
    _debugW(RuleMessageTemplatesTableV1.getClassName(), '_addTemplate', {
      template
    });
  ...
  private _refreshTemplateIndexes() {
    ...
    _debugW(RuleMessageTemplatesTableV1.getClassName(), '_refreshTemplateIndexes', {
