...
*/
const MODULE_ID = 'aca-classification-rules-service-classification-rule-delete-many-by-ids';
...
const _retrieveRules = async (context, params) => {
  const CONTEXT_USER_ID = context?.user?.id;
  ...
  try {
    ...
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    appendDataToError(ACA_ERROR, { CONTEXT_USER_ID });
    logger.error(`${_retrieveRules.name}`, { ACA_ERROR });
    throw ACA_ERROR;
  }
}
  ...
  try {
    ...
    if (
      ...
      for (let ruleId of PARAMS_IDS) {
        ...
        PROMISES.push(DATASOURCE.rulesClassifications.deleteManyByRuleId(context, { ruleId }));
      }
    }
    await Promise.all(PROMISES);
    getEventStreamByContext(context).publish(AIAP_EVENT_TYPE.RESET_CLASSIFICATION_RULE_ENGINES, {});
    return RET_VAL;
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    appendDataToError(ACA_ERROR, { CONTEXT_USER_ID });
...
const deleteManyByIds = async (context, params) => {
  ...
  let rules;
  try {
    rules = await _retrieveRules(context, params);
