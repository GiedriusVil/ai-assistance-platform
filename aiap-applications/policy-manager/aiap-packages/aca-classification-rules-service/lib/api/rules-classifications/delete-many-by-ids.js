...
*/
const MODULE_ID = 'aca-classification-rules-service-classification-rule-classifications-delete-many-by-ids';
...
const _saveAuditRecord = async (context, params) => {
  ...
  try {
    ...
    value = {
      ...
      docType: 'CLASSIFICATION_RULE_CLASSIFICATIONS',
      doc: doc,
    }
...
const _retrieveClassifications = async (context, params) => {
  const CONTEXT_USER_ID = context?.user?.id;
  ...
  try {
    ...
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    appendDataToError(ACA_ERROR, { CONTEXT_USER_ID });
    logger.error(`${_retrieveClassifications.name}`, { ACA_ERROR });
    throw ACA_ERROR;
  }
}
...
const _deleteManyByIds = async (context, params) => {
  ...
  try {
    ...
    const RET_VAL = await DATASOURCE.rulesClassifications.deleteManyByIds(context, params);
    return RET_VAL;
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    appendDataToError(ACA_ERROR, { CONTEXT_USER_ID });
...
const deleteManyByIds = async (context, params) => {
  ...
  let classifications;
  try {
    classifications = await _retrieveClassifications(context, params);
    const RET_VAL = await executeEnrichedByLambdaModule(MODULE_ID, _deleteManyByIds, context, params);
    await _saveAuditRecords(context, classifications);
    return RET_VAL;
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
