...
*/
const MODULE_ID = 'aca-rule-actions-service-rules-actions-delete-many-by-ids';
...
const _retrieveActions = async (context, params) => {
  const CONTEXT_USER_ID = context?.user?.id;
  ...
  try {
    ...
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    appendDataToError(ACA_ERROR, { CONTEXT_USER_ID });
    logger.error(`${_retrieveActions.name}`, { ACA_ERROR });
    throw ACA_ERROR;
  }
}
...
const _deleteManyByIds = async (context, params) => {
  ...
  try {
    ...
    const RET_VAL = await DATASOURCE.actions.deleteManyByIds(context, params);

    await runtimeDataService.deleteManyByIdsFromConfigDirectory(context, params);
