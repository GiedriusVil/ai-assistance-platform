...
*/
const MODULE_ID = 'aca-rules-actions-service-runtime-data-delete-many-by-ids-from-config-directory';
...
const deleteManyByIdsFromConfigDirectory = async (context, params) => {
  ...
  try {
    ...
    if (
      ...
      const DIR_RULES_ACTIONS = `${configAbsolutePath}/runtime-data-local/policy-manager/${context?.user?.session?.tenant?.id}/rules-actions`;
      fsExtra.ensureDirSync(DIR_RULES_ACTIONS);
      if (
    ...
    logger.error(deleteManyByIdsFromConfigDirectory.name, { ACA_ERROR });
    throw ACA_ERROR;
  }
}

export {
  deleteManyByIdsFromConfigDirectory,
}
