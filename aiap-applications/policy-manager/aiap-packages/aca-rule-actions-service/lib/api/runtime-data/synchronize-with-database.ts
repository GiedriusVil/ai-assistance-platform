...
*/
const MODULE_ID = 'aca-rule-actions-service-runtime-data-synchronize-with-database';
...
import { synchronizeWithDatabaseByTenant } from './synchronize-with-database-by-tenant';
...
const _synchronizeWithDatabaseByTenant = async (context, params) => {
  ...
  try {
    ...
    const RULES_ACTIONS = await synchronizeWithDatabaseByTenant(CONTEXT, params);
    const RET_VAL = {
      tenantId: params?.tenantId,
      rulesActions: RULES_ACTIONS,
    };
    return RET_VAL;
  } catch (error) {
    ...
    if (
      ...
      if (
        ...
        for (const RESULT of PROMISES_RESULTS) {
          logger.info(`Policy Manager Rules Actions synchronized [TenantId: ${RESULT?.tenantId}, QTY.: ${RESULT?.rulesActions?.length}]`);
        }
      }
    }
  } catch (error) {
