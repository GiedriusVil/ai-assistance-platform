...
*/
const MODULE_ID = 'aca-buy-rules-express-routes-controllers-rules-import-many-from-file';
...
const importMany = async (request, response) => {
    ...
    try {
        ...
        file = request?.files?.buyRuleFile;
        if (
        ...
        if (
            ...
            const MESSAGE = 'Missing requred request.files.buyRuleFile parameter!';
            throwAcaError(MODULE_ID, ACA_ERROR_TYPE.VALIDATION_ERROR, MESSAGE);
        }
        params = { file };
