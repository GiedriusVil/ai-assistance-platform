...
*/
const MODULE_ID = 'aca-catalog-rules-service-rules-catalogs-find-many-by-query';
...
const _findManyByQuery = async (context, params) => {
    ...
    try {
        ...
        const RET_VAL = await DATASOURCE.rulesCatalogs.findManyByQuery(context, params);
        return RET_VAL;
    } catch (error) {
        const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
        appendDataToError(ACA_ERROR, { CONTEXT_USER_ID });
        logger.error(`${_findManyByQuery.name}`, { CONTEXT_USER_ID });
        throw ACA_ERROR;
    }
}
...
const findManyByQuery = async (context, params) => {
    ...
    try {
        ...
        const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
        appendDataToError(ACA_ERROR, { CONTEXT_USER_ID });
        logger.error(`${findManyByQuery.name}`, { CONTEXT_USER_ID });
        throw ACA_ERROR;
    }
};
