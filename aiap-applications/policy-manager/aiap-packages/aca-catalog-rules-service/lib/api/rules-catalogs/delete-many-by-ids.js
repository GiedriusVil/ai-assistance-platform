...
*/
const MODULE_ID = 'aca-catalog-rules-service-catalogs-delete-many-by-ids';
...
const _retrieveCatalogs = async (context, params) => {
  const CONTEXT_USER_ID = context?.user?.id;
  ...
  try {
    ...
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    appendDataToError(ACA_ERROR, { CONTEXT_USER_ID });
    logger.error(`${_retrieveCatalogs.name}`, { ACA_ERROR });
    throw ACA_ERROR;
  }
}
...
const _deleteManyByIds = async (context, params) => {
  ...
  try {
    ...
    const RET_VAL = await DATASOURCE.rulesCatalogs.deleteManyByIds(context, params);
    return RET_VAL;
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    appendDataToError(ACA_ERROR, { CONTEXT_USER_ID });
