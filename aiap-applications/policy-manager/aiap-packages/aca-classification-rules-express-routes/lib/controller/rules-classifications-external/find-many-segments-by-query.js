...
*/
const MODULE_ID = 'aca-classification-rules-express-routes-controller-classification-rules-classifications-external-find-many-segments-by-query';
...
const findManySegmentsByQuery = async (request, response) => {
  const ERRORS = [];
  let params;
  let context;
  let result;
  try {
    ...
    result = await rulesClassificationsExternalService.findManySegmentsByQuery(context, params);
  } catch (error) {
  ...
  if (lodash.isEmpty(ERRORS)) {
    ...
    logger.error(`${findManySegmentsByQuery.name}`, { errors: ERRORS });
