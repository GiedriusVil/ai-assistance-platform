...
*/
const MODULE_ID = 'aca-classification-rules-express-routes-controller-classification-rules-classifications-external-find-many-families-by-query';
...
const findManyFamiliesByQuery = async (request, response) => {
  const ERRORS = [];
  let params;
  let context;
  let result;
  try {
    ...
    result = await rulesClassificationsExternalService.findManyFamiliesByQuery(context, params);
  } catch (error) {
  ...
  if (lodash.isEmpty(ERRORS)) {
    ...
    logger.error(`${findManyFamiliesByQuery.name}`, { errors: ERRORS });
