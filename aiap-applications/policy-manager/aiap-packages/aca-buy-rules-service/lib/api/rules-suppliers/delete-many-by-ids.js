...
*/
const MODULE_ID = 'aca-buy-rules-service-rule-suppliers-delete-many-by-ids';
...
const _retrieveSuppliers = async (context, params) => {
  const CONTEXT_USER_ID = context?.user?.id;
  ...
  try {
    ...
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    appendDataToError(ACA_ERROR, { CONTEXT_USER_ID });
    logger.error(`${_retrieveSuppliers.name}`, { ACA_ERROR });
    throw ACA_ERROR;
  }
}
...
const _deleteManyByIds = async (context, params) => {
  ...
  try {
    ...
    const RET_VAL = await DATASOURCE.rulesSuppliers.deleteManyByIds(context, params);
    return RET_VAL;
  } catch (error) {
    const ACA_ERROR = formatIntoAcaError(MODULE_ID, error);
    appendDataToError(ACA_ERROR, { CONTEXT_USER_ID });
