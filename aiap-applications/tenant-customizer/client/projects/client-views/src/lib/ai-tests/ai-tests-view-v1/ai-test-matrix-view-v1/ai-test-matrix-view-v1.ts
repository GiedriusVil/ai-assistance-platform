...
@Component({
  selector: 'aca-aitest-matrix-view',
  templateUrl: './ai-test-matrix-view-v1.html',
  styleUrls: ['./ai-test-matrix-view-v1.scss']
})
export class AiTestMatrixView extends BaseViewV1 implements OnInit, OnDestroy, AfterViewInit {

  static getClassName() {
    return 'AiTestMatrixView';
  }
  ...
  addFilterEventHandler() {
    ...
    this.aiTestsService.findOneById(this.testId).pipe(
      ...
      _debugX(AiTestMatrixView.getClassName(), `addFilterEventHandler`,
        {
          response,
        });
  ...
  subscribeToQueryParams() {
    this.ActivatedRouteServiceV1.queryParams()
      .subscribe((params: any) => {
        _debugX(AiTestMatrixView.getClassName(), 'subscribeToQueryParams',
          {
            params,
          });
  ...
  static route() {
    const RET_VAL = {
      path: 'ai-test-confusion-matrix',
      component: AiTestMatrixView,
      data: {
        breadcrumb: 'ai_test_matrix_view_v1.breadcrumb',
        component: AiTestMatrixView.getClassName(),
        actions: []
      }
    };
    return RET_VAL;
