...
*/
import { Component, OnInit, EventEmitter, Output, ViewChild, TemplateRef, OnDestroy, Input, OnChanges } from '@angular/core';
...
@Component({
  selector: 'aca-engagements-table',
  templateUrl: './engagements-table.html',
  styleUrls: ['./engagements-table.scss'],
})
export class EngagementsTable extends BaseTable implements OnInit, OnDestroy {
  ...
  state: any = {
    queryType: DEFAULT_TABLE.ENGAGEMENTS.TYPE,
    defaultSort: DEFAULT_TABLE.ENGAGEMENTS.SORT,
    search: '',
  };
  ...
  constructTableHeader() {
    ...
    TABLE_HEADER.push(new TableHeaderItem({ data: 'Name', field: 'name' }));
    TABLE_HEADER.push(new TableHeaderItem({ data: 'Assistant', field: 'assistant' }));
    TABLE_HEADER.push(new TableHeaderItem({ data: 'Test', field: 'id', sortable: false }));
  ...
  transformResponseItemToRow(item) {
    ...
    RET_VAL.push(new TableItem({ data: item?.assistant.id }));
  ...
  addFilterEventHandler() {
    ...
    this.eventsService.filterEmitter.pipe(
      ...
      _debugX(EngagementsTable.getClassName(), `addFilterEventHandler`, { response });
  ...
  handleFindManyByQueryError(error: any) {
    _errorX(EngagementsTable.getClassName(), `handleFindManyByQueryError`, { error });
    this.eventsService.loadingEmit(false);
  ...
  handleOpenEngagementChatInNewTab(data: any = undefined) {
    _debugX(EngagementsTable.getClassName(), 'handleOpenEngagementChatInNewTab', { data });
