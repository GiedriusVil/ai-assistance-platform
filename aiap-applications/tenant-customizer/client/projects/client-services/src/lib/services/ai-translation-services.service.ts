...
@Injectable()
export class AiTranslationServicesService extends BaseServiceV1 {

  static getClassName() {
    return 'AiTranslationServicesService';
  }
  ...
  findManyByQuery(query: any) {
    ...
    _debugX(AiTranslationServicesService.getClassName(), 'findManyByQuery', { REQUEST_URL, REQUEST, REQUEST_OPTIONS });
  ...
  saveOne(aiTranslationService: any) {
    ...
    const REQUEST_OPTIONS = this._requestOptions();
    _debugX(AiTranslationServicesService.getClassName(), 'saveOne', { REQUEST_URL, REQUEST, REQUEST_OPTIONS });
    const RET_VAL = this.http.post(REQUEST_URL, REQUEST, REQUEST_OPTIONS);
  ...
  deleteManyByIds(ids: Array<any>) {
    ...
    const REQUEST = { ids };
    const REQUEST_OPTIONS = this._requestOptions();
    _debugX(AiTranslationServicesService.getClassName(), 'deleteManyByIds', { REQUEST_URL, REQUEST, REQUEST_OPTIONS });
    const RET_VAL = this.http.post(REQUEST_URL, REQUEST, REQUEST_OPTIONS);
  ...
  exportMany(query: any) {
    ...
    _debugX(AiTranslationServicesService.getClassName(), 'exportMany', { REQUEST_URL });
  ...
  importManyFromFile(file: File) {
    ...
    const REQUEST_OPTIONS = this._requestOptions();
    _debugX(AiTranslationServicesService.getClassName(), 'importManyFromFile', { REQUEST_URL, REQUEST, REQUEST_OPTIONS });
    const RET_VAL = this.http.post(REQUEST_URL, REQUEST, REQUEST_OPTIONS);
  ...
  loadAiTranslationServiceFormData() {
    ...
    const TYPES = [
      {
        content: 'Watson Language Translator',
        type: 'WLT',
      },
      {
        name: 'WatsonX',
