...
@Injectable()
export class AiTestsService extends BaseServiceV1 {

  static getClassName() {
    return 'AiTestsService';
  }
  ...
  findManyLiteByQuery(query: any) {
    ...
    _debugX(AiTestsService.getClassName(), 'findManyLiteByQuery', { REQUEST_URL, REQUEST, REQUEST_OPTIONS });
  ...
  saveOne(params: any) {
    ...
    return this.http.post(REQUEST_URL, params, this.getAuthHeaders());
  }

  deleteOne(id: string) {
    const REQUEST_URL = `${this._hostUrl()}/api/v1/ai/tests/${id}`;
    return this.http.delete(REQUEST_URL, this.getAuthHeaders());
  }
