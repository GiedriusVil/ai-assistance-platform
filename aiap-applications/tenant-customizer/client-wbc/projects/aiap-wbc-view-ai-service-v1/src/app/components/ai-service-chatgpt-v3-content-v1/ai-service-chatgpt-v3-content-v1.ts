...
@Component({
  selector: 'aiap-ai-service-chatgpt-v3-content-v1',
  templateUrl: './ai-service-chatgpt-v3-content-v1.html',
  styleUrls: ['./ai-service-chatgpt-v3-content-v1.scss'],
})
export class AiServiceChatGptV3ContentV1 extends BaseViewV1 implements OnInit, OnDestroy, AfterViewInit, AfterViewChecked {
  
  static getClassName() {
    return 'AiServiceChatGptV3ContentV1';
  }
  ...
  handleShowSavePlace(event: any) {
    _debugX(AiServiceChatGptV3ContentV1.getClassName(), 'handleShowSavePlace', { event });
  ...
  handleShowDeletePlace(event: any) {
    _debugX(AiServiceChatGptV3ContentV1.getClassName(), 'handleShowDeletePlace', { event });
    this.aiSkillDeleteModal.show(this.aiService.id, event);
  }

  handleShowSyncPlaceEvent(event: any) {
    _debugX(AiServiceChatGptV3ContentV1.getClassName(), 'handleShowSyncPlaceEvent', { event });
    this.aiSkillSyncModal.show(this.aiService.id, event);
  }

  handleShowSyncByFilePlaceEvent(event: any) {
    _debugX(AiServiceChatGptV3ContentV1.getClassName(), 'handleShowSyncByFilePlaceEvent', { event });
    this.aiSkillSyncByFileModal.show(this.aiService.id);
  }

  handleShowPullPlaceEvent(event: any) {
    _debugX(AiServiceChatGptV3ContentV1.getClassName(), 'handleShowPullPlaceEvent', { event });
    this.aiSkillPullModal.show(this.aiService.id, event);
  }

  handleSearchChangeEvent(event: any) {
    _debugX(AiServiceChatGptV3ContentV1.getClassName(), `handleSearchChangeEvent`, { event });
    ...
    _debugX(AiServiceChatGptV3ContentV1.getClassName(), `handleSearchChangeEvent`, { QUERY });
    this.eventsService.filterEmit(QUERY);
  }


  handleSearchClearEvent(event: any) {
    _debugX(AiServiceChatGptV3ContentV1.getClassName(), `handleSearchClearEvent`, { event });
    this.queryService.deleteFilterItems(DEFAULT_TABLE.AI_SKILLS_V1.TYPE, QueryServiceV1.FILTER_KEY.SEARCH);
    const QUERY = this.queryService.query(DEFAULT_TABLE.AI_SKILLS_V1.TYPE);
    _debugX(AiServiceChatGptV3ContentV1.getClassName(), `handleSearchClearEvent`, { QUERY });
    this.eventsService.filterEmit(QUERY);
  }
  ...
  showAiSkillDeleteModal(aiSkill: any) {
    ...
    _debugX(AiServiceChatGptV3ContentV1.getClassName(), 'emitShowAiSkillDeleteModal', { EVENT });
