...
@Component({
  selector: 'aiap-ai-translation-service-external-watsonx-save-form-v1',
  templateUrl: './ai-translation-service-external-watsonx-save-form-v1.html',
  styleUrls: ['./ai-translation-service-external-watsonx-save-form-v1.scss'],
  viewProviders: [{
    provide: ControlContainer,
    useExisting: NgForm
  }]
})
export class AiTranslationServiceExternalWatsonxSaveFormV1 implements OnInit, OnDestroy, OnChanges {

  static getClassName() {
    return 'AiTranslationServiceExternalWatsonxSaveFormV1';
  }
  ...
  isIamTokenUrlFieldDisabled() {
    let retVal = true;
    if (
      this.state.selections?.authType &&
      this.state.selections?.authType?.type === 'iam'
    ) {
      retVal = false;
    }
    return retVal;
  ...
  handleAuthTypeSelectedEvent(event: any) {
    _debugX(AiTranslationServiceExternalWatsonxSaveFormV1.getClassName(), 'onAuthTypeSelected', {
  ...
  handleShowApiKeyClickEvent(event: any) {
    _debugX(AiTranslationServiceExternalWatsonxSaveFormV1.getClassName(), 'handleShowApiKeyClickEvent', {
      event: event,
