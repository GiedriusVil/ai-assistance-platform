...
@Component({
  selector: 'aiap-ai-translation-model-import-modal-v1',
  templateUrl: './ai-translation-model-import-modal-v1.html',
  styleUrls: ['./ai-translation-model-import-modal-v1.scss'],
})
export class AiTranslationModelImportModalV1 extends BaseModalV1 implements OnInit, OnDestroy, AfterViewInit {

  static getClassName() {
    return 'AiTranslationModelImportModalV1';
  }
  ...
  async import() {
    ...
    _debugX(AiTranslationModelImportModalV1.getClassName(), 'import', { FILE });
    this.aiTranslationModelsService.importManyFromFile(FILE, PARAMS).pipe(
      tap(() => this.eventsService.loadingEmit(true)),
      catchError((error) => this.handleImportManyFromFile(error)),
      takeUntil(this._destroyed$),
    ).subscribe((response: any) => {
      _debugX(AiTranslationModelImportModalV1.getClassName(), 'import', { response });
  ...
  handleImportManyFromFile(error: any) {
    _errorX(AiTranslationModelImportModalV1.getClassName(), 'handleImportError', { error });
    this.eventsService.loadingEmit(false);
