...
@Injectable()
export class TopicModelingServiceV1 extends BaseServiceV1 {

  static getClassName() {
    return 'TopicModelingServiceV1';
  }
  ...
  _hostAndBasePath() {
    const RET_VAL = `${this._hostUrl()}/api/v1/topic-modeling`;
    return RET_VAL;
  }
  ...
  findManyByQuery(query: any) {
    ...
    _debugX(TopicModelingServiceV1.getClassName(), 'findManyByQuery',
      {
        REQUEST_URL,
        REQUEST,
        REQUEST_OPTIONS,
  ...
  saveOne(value: any) {
    ...
    const REQUEST = { value };
    const REQUEST_OPTIONS = this._requestOptions();
    _debugX(TopicModelingServiceV1.getClassName(), 'saveOne',
      {
        REQUEST_URL,
        REQUEST,
        REQUEST_OPTIONS,
  ...
  getSummaryByQuery(query) {
    ...
    const REQUEST = { query };
    const REQUEST_OPTIONS = this._requestOptions();
    _debugX(TopicModelingServiceV1.getClassName(), 'getSummaryByQuery',
      {
        REQUEST_URL,
        REQUEST,
        REQUEST_OPTIONS,
  ...
  executeJobById(id) {
    ...
    _debugX(TopicModelingServiceV1.getClassName(), 'executeJobById', {
      REQUEST_URL,
      REQUEST,
      REQUEST_OPTIONS,
    });
