...
@Component({
  selector: 'aiap-live-analytics-tile-metric-v2',
  templateUrl: './live-analytics-tile-metric-v2.html',
  styleUrls: ['./live-analytics-tile-metric-v2.scss'],
})
export class LiveAnalyticsTileMetricV2 implements OnInit, OnDestroy {

  static getClassName() {
    return 'LiveAnalyticsTileMetricV2';
  }
  ...
  private addFilterEventHandler() {
    ...
    this.filtersSubscription = this.eventsService.filterEmitter.pipe(
      ...
      _debugX(LiveAnalyticsTileMetricV2.getClassName(), 'filterEventHandler', { response });
  ...
  private loadTile() {
    ...
    this.tilesService.findOneByRef(this.tileRef).pipe(
      ...
      _debugX(LiveAnalyticsTileMetricV2.getClassName(), 'loadTile', { response });
      this.refreshTile(response);
    });
  }
  ...
  private handleRetrieveTileMetricDataError(error: any) {
    _errorX(LiveAnalyticsTileMetricV2.getClassName(), 'handleRetrieveTileMetricDataError', { error });
  ...
  private loadTileMetricData() {
    ...
    _debugX(LiveAnalyticsTileMetricV2.getClassName(), 'loadTileMetricData', PARAMS);
    this.liveAnalyticsService.executeOne(PARAMS)
      .pipe(
        catchError((error: any) => this.handleRetrieveTileMetricDataError(PARAMS))
      ).subscribe((response: any) => {
        _debugX(LiveAnalyticsTileMetricV2.getClassName(), 'loadTileMetricData', { response });
        const NEW_STATE = lodash.cloneDeep(this.state);
  ...
  private handleExportDataEvent () {
    ...
    _debugX(LiveAnalyticsTileMetricV2.getClassName(), 'exportTileMetricData', PARAMS);
    this.liveAnalyticsService.exportOne(PARAMS)
      .pipe(
        catchError((error: any) => this.handleRetrieveTileMetricDataError({error}))
      ).subscribe((response: any) => {
        _debugX(LiveAnalyticsTileMetricV2.getClassName(), 'exportTileMetricData', { response });
