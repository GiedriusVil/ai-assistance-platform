...
export class QueryServiceV1 extends BaseServiceV1 {
  ...
  public setFilterItem(type: string, key: string, item: any) {
    ...
    if (
      ...
      _warnX(QueryServiceV1.getClassName(), 'setFilterItem', { key, item });
    }
  ...
  public getSearchValue(type: string) {
    _debugX(QueryServiceV1.getClassName(), `getSearchValue `, { type });
    const QUERY = this._retrieveQuery(type);
  ...
  private _saveQuery(type: string, query: any) {
    _debugX(QueryServiceV1.getClassName(), `_saveQuery `, { type, query });
    const QUERY_TO_SAVE = {
      [type]: query
    }
  ...
  private _setQueryToUrl(query: any) {
    ...
    for (const KEY of Object.keys(HTTP_QUERY_PARAMS)) {
      if (
        KEY !== 'query'
      ) {
        const HTTP_QUERY_PARAM_VALUE = HTTP_QUERY_PARAMS[KEY];
  ...
  private _getQueryFromUrl() {
    ...
    _debugX(QueryServiceV1.getClassName(), `_getQueryFromUrl `, { RET_VAL, QUERY_ENCODED });
    return RET_VAL;
  }
  ...
  private _getQueryFromLocalStorage() {
    ...
    _debugX(QueryServiceV1.getClassName(), `_getQueryFromLocalStorage`, { RET_VAL });
    return RET_VAL;
  }
