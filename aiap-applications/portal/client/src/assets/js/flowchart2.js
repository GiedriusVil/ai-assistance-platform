...
function pathflowDraw(container, config, data) {
  //debugger;
  ...
  function update(source) {
    ...
    nodeObjects.on("mouseover", function (d) {
      ...
      _tooltip.html(
        '<div>' +
        '<p><strong>Intent: </strong>' + d.data.name +
        '<br>' +
        '<p><strong>Skill: </strong>' + d.data.skill +
        '<br>' +
        '<strong>Messages: </strong>' + d.data.flows + " (" + (d.data.flowRatio * 100).toFixed(1) + "%)" +
        '<br>' +
        '<strong>Exits: </strong>' + d.data.exits + " (" + (d.data.exitRatio * 100).toFixed(1) + '%)' +
        '<br>' +
        '<strong>Not handled: </strong>' + d.data.not_handled + " (" + (d.data.notHandledRatio * 100).toFixed(1) + '%)' +
        '<br>' +
        '</p></div>')
  ...
  function stratifyJSONArray(data) {
    ...
    var rootNode = { path: _config.commonRootPathName, id: _config.commonRootPathName, "name": _config.commonRootPathName, type: "start", is_new_skill: 0, is_session_start: 0, dropped_off: 0, rerouted: 0, flows: 0, exits: 0, not_handled: 0, dropped_offRatio: 0, reroutedRatio: 0, exitRatio: 0, flowRatio: 100, flowRationFromPrev: 100, notHandledRatio: 0 };
    ...
    root.each(function (node) {
      ...
      node.data.exitRatio = node.data.exits / node.data.flows;
  ...
  function updateInfoDiv() {
    ...
    d3.select(_chart.container).select(".infodiv-node-messages").html("<span class='field-name'>Messages: </span>" + node.flows + " (" + (node.flowRatio * 100).toFixed(1) + "% of total)");
    d3.select(_chart.container).select(".infodiv-node-exits").html("<span class='field-name'>Exits: </span>" + node.exits + " (" + (node.exitRatio * 100).toFixed(1) + "%)");
    d3.select(_chart.container).select(".infodiv-node-not_handled").html("<span class='field-name'>Not handled: </span>" + node.not_handled + " (" + (node.notHandledRatio * 100).toFixed(1) + "%)");
  }
  ...
  function createInfoDiv(_chart) {
    ...
    $('<div>', { text: 'Messages: ' }).addClass("infodiv-node-messages").appendTo(infodiv);
    $('<div>', { text: 'Exists: ' }).addClass("infodiv-node-exits").appendTo(infodiv);
    $('<div>', { text: 'Not Handled: ' }).addClass("infodiv-node-not_handled").appendTo(infodiv);
