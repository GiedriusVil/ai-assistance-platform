...
export class AttachmentsServiceV1 {
  ...
  isAttachmentVisibleButtons(message: any): boolean {
    ...
    const RET_VAL = this._isBotMessage(message) && TYPE === ATTACHMENT_TYPES.BUTTONS;
    return RET_VAL;
  }
  ...
  isAttachmentVisibleFeedback(message: any): boolean {
    const SCORE = ramda.pathOr(undefined, ['feedbackScore'], message);
    const TYPE = ramda.pathOr(undefined, ['sender_action', 'type'], message);
    const RET_VAL = SCORE || TYPE === ATTACHMENT_TYPES.FEEDBACK;
    return RET_VAL;
  }
  ...
  isAttachmentVisibleIntentsMenu(message: any): boolean {
    const TYPE = this._attachmentType(message);
    const RET_VAL = this._isBotMessage(message) && TYPE === ATTACHMENT_TYPES.INTENTS_MENU;
    return RET_VAL;
  }
