...
@Injectable()
export class ZendeskLiveAgentServiceV1 {
  ...
  public botSocketIoService: BotSocketIoServiceV1;
  ...
  static getClassName() {
    return 'ZendeskLiveAgentServiceV1';
  }
  ...
  getBotSocketIoService(): BotSocketIoServiceV1 {
    return this.injector.get<BotSocketIoServiceV1>(BotSocketIoServiceV1);
  }
  ...
  connect(apiKey: string): void {
    ...
    _infoX(ZendeskLiveAgentServiceV1.getClassName(), 'connect');
  ...
  connectionUpdate(status: string): void {
    switch (status) {
      case Z_CHAT_CONNECTION_STATUS.CONNECTED:
        _infoX(ZendeskLiveAgentServiceV1.getClassName(), 'connectionUpdate', Z_CHAT_CONNECTION_STATUS.CONNECTED);

        this.checkAccountStatus();
        break;

      case Z_CHAT_CONNECTION_STATUS.CONNECTING:
        _infoX(ZendeskLiveAgentServiceV1.getClassName(), 'connectionUpdate', Z_CHAT_CONNECTION_STATUS.CONNECTING);
        break;

      case Z_CHAT_CONNECTION_STATUS.CLOSED:
        _infoX(ZendeskLiveAgentServiceV1.getClassName(), 'connectionUpdate', Z_CHAT_CONNECTION_STATUS.CLOSED);
        break;
    }
  }
  ...
  checkAccountStatus(): void {
    ...
    switch (ACCOUNT_STATUS) {
      case Z_CHAT_ACCOUNT_STATUS.ONLINE:
        _infoX(ZendeskLiveAgentServiceV1.getClassName(), 'checkAccountStatus', Z_CHAT_ACCOUNT_STATUS.ONLINE);
      ...
      case Z_CHAT_ACCOUNT_STATUS.AWAY:
        _infoX(ZendeskLiveAgentServiceV1.getClassName(), 'checkAccountStatus', ACCOUNT_STATUS);
    ...
    try {
      ...
      if (DEPARTMENTS) {
        ...
        if (ONLINE_DEPARTMENTS) {
          ...
          _infoX(ZendeskLiveAgentServiceV1.getClassName(), 'getAvailableDepartments', ONLINE_DEPARTMENTS);
        } else {
      ...
      _errorX(ZendeskLiveAgentServiceV1.getClassName(), 'getAvailableDepartments', error);
    }
  }
  ...
  postUserMessage(message: string): void {
    ...
    _infoX(ZendeskLiveAgentServiceV1.getClassName(), 'postUserMessage', message);
  }

  chat(event_data: any): void {
    _infoX(ZendeskLiveAgentServiceV1.getClassName(), 'chat', event_data);
  ...
  removeListeners(eventType: string): void {
    zChat.un(eventType, () => {
      _infoX(ZendeskLiveAgentServiceV1.getClassName(), `removeListeners`, eventType);
    });
  }
  ...
  endLiveChat(): void {
    ...
    zChat.endChat(CLEAR_DEPARTMENT, () => {
      _infoX(ZendeskLiveAgentServiceV1.getClassName(), 'endLiveChat');
