...
const EVENT_TYPE: any = {
  ...
  NOT_IN_CATALOG_BTN_PRESSED: 'notInCatalogBtnPressed',
}

@Component({
  selector: 'aca-product-list',
...
export class ProductList implements OnInit {
  ...
  @Input() notInCatalogMessage: string;

  @Output() onProductEvent = new EventEmitter<any>();
  @Output() onNotInCatalogBtnEvent = new EventEmitter<any>();

  collectionSize: number;

  _state = {
    products: [],
    notInCatalogMessage: null,
    productsTotalQty: 0,
  ...
  ngOnInit(): void {
    ...
    this._state.notInCatalogMessage = this.notInCatalogMessage;
  ...
  handleNotInCatalogBtnEvent() {
    const EVENT = {
      type: EVENT_TYPE.NOT_IN_CATALOG_BTN_PRESSED,
    };
    this.onNotInCatalogBtnEvent.emit(EVENT);
    this._state.enable.notInCatalogBtn = false;
  }
  ...
  isNotInCatalogBtnVisible(): boolean {
    let retValue = false;
    if (this._state.notInCatalogMessage) {
      retValue = true;
    }
    return retValue;
  }
