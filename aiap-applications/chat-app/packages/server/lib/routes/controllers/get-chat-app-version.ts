...
*/
const MODULE_ID = 'aca-chat-app-server-controller-get-chat-app-version';
...
const getChatAppVersion = async (request, response) => {
  const ERRORS = [];
  let retVal;
  try {
  ...
  if (
    ...
    logger.error(getChatAppVersion.name, { ERRORS });
