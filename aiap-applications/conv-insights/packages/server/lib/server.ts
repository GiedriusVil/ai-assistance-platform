...
import configurationProvider from '@ibm-aiap/aiap-conv-insights-configuration';
...
const startServer = async () => {
  ...
  await require('@ibm-aca/aca-topic-modeling-express-routes').initByConfigurationProvider(configurationProvider, APP);
  await require('@ibm-aiap/aiap-ai-services-express-routes').initByConfigurationProvider(configurationProvider, APP);

  APP.use(
    '/docs',
    swaggerUi.serve, swaggerUi.setup(require('./docs/swagger'))
  );
