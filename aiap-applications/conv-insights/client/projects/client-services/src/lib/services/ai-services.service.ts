...
@Injectable()
export class AiServicesService extends BaseServiceV1 {

  static getClassName() {
    return 'AiServicesService';
  }
  ...
  findManyByQuery(query: any) {
    ...
    _debugX(AiServicesService.getClassName(), 'findManyByQuery', { REQUEST_URL, REQUEST, REQUEST_OPTIONS });
  ...
  syncOneById(id: any) {
    ...
    const REQUEST = { id };
    const REQUEST_OPTIONS = this._requestOptions();
    _debugX(AiServicesService.getClassName(), 'refreshOneById', { REQUEST_URL, REQUEST, REQUEST_OPTIONS });
    const RET_VAL = this.http.post(REQUEST_URL, REQUEST, REQUEST_OPTIONS);
    return RET_VAL;
  }

  saveOne(aiService: any, options: any = undefined) {
    const REQUEST_URL = `${this._hostAndBasePath()}/save-one`;
    const REQUEST = { aiService, options };
    const REQUEST_OPTIONS = this._requestOptions();
    _debugX(AiServicesService.getClassName(), 'saveOne', { REQUEST_URL, REQUEST, REQUEST_OPTIONS });
    const RET_VAL = this.http.post(REQUEST_URL, REQUEST, REQUEST_OPTIONS);
    return RET_VAL;
  }
  ...
  deleteManyByIds(ids: Array<any>) {
    ...
    const REQUEST = { ids };
    const REQUEST_OPTIONS = this._requestOptions();
    _debugX(AiServicesService.getClassName(), 'deleteManyByIds', { REQUEST_URL, REQUEST, REQUEST_OPTIONS });
    const RET_VAL = this.http.post(REQUEST_URL, REQUEST, REQUEST_OPTIONS);
    return RET_VAL;
  }
  ...
  exportMany(query: any) {
    ...
    _debugX(AiServicesService.getClassName(), 'exportMany', { REQUEST_URL });
  ...
  importManyFromFile(file: File) {
    ...
    const REQUEST_OPTIONS = this._requestOptions();
    _debugX(AiServicesService.getClassName(), 'importManyFromFile', { REQUEST_URL, REQUEST, REQUEST_OPTIONS });
    const RET_VAL = this.http.post(REQUEST_URL, REQUEST, REQUEST_OPTIONS);
    return RET_VAL;
  }
  ...
  retrievePullOptions(assistant: any) {
    ...
    const REQUEST_OPTIONS = this._requestOptions();
    _debugX(AiServicesService.getClassName(), 'retrievePullOptions', { REQUEST_URL, REQUEST, REQUEST_OPTIONS });
    const RET_VAL = this.http.post(REQUEST_URL, REQUEST, REQUEST_OPTIONS);
    return RET_VAL;
  }
  ...
  rollbackAiSkill(aiServiceId: any, aiSkillReleaseId: any) {
    ...
    _debugX(AiServicesService.getClassName(), 'rollbackAiSkill', { REQUEST_URL, REQUEST_OPTIONS });
