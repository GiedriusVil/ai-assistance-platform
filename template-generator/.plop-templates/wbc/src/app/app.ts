...
const HTML_TAG = 'aiap-wbc-view-{{dashCase name}}-v{{version}}';

@Component({
  selector: 'aiap-wbc-view-{{dashCase name}}-v{{version}}',
  templateUrl: './app.html',
  styleUrls: ['./app.scss'],
})
export class AiapWbcView{{properCase name}}V{{version}} extends BaseAppWbcViewV1 implements OnInit, OnChanges {

  static getClassName() {
    return `AiapWbcView{{properCase name}}V{{version}}`;
  }
  ...
  async ngOnChanges(changes: SimpleChanges) {
    ...
    try {
      ...
      _debugW(AiapWbcView{{properCase name}}V{{version}}.getClassName(), 'ngOnChanges', { changes });
      if (
      ...
      _errorW(AiapWbcView{{properCase name}}V{{version}}.getClassName(), 'ngOnChanges', { wbc });
      throw error;
    }
  }
  ...
  protected elCSSLinkId() {
    const RET_VAL = AiapWbcView{{properCase name}}V{{version}}.getElementTag();
    return RET_VAL;
  }
