...
# - lists all existing Helm releases in the specified namespace and defines new release name;
# - downloads required configs based on specified ENV variables
# - deploys Helm chart into specified cluster/namespace.
...
if [ -z "${TILLER_VERSION}" ]; then
  ...
  echo -e "Helm Tiller v${TILLER_VERSION} already installed in cluster. Keeping it, and aligning it with client version."
...
# install Tiller based on local Helm client version
if [[ $(echo $CLIENT_VERSION | cut -d. -f1) -lt 3  && -z $TILLER_VERSION ]]; then
  echo "Installing Helm Tiller v${CLIENT_VERSION} with cluster admin privileges RBAC"
  kubectl -n kube-system create serviceaccount tiller
  kubectl create clusterrolebinding tiller --clusterrole cluster-admin --serviceaccount=kube-system:tiller
  helm init --service-account tiller ${HELM_TILLER_TLS_OPTION}
  kubectl -n kube-system rollout status deploy/tiller-deploy
else
  echo "Skipping Tiller installation"
fi

helm version ${HELM_TLS_OPTION}

echo "=========================================================="
echo "ADDING AIDT-CHARTS REPOSITORY"
helm repo add aidt-charts ${AIDT_CHARTS_REPO} --username ${AIDT_CHARTS_USER} --password ${AIDT_CHARTS_PASSWORD}

echo "=========================================================="
echo -e "CHECKING HELM RELEASES IN THE NAMESPACE: ${CLUSTER_NAMESPACE}"
helm list ${HELM_TLS_OPTION} -n ${CLUSTER_NAMESPACE}

echo "=========================================================="
echo "DEFINING RELEASE NAME"
if [ -z "$RELEASE_NAME" ]; then
  if [[ "${CLUSTER_NAMESPACE}" != "default" && -n $TILLER_VERSION ]]; then
    echo "Prefixing image (app) name with namespace if not 'default' as Helm2 needs unique release names across namespaces"
...
if [ "${HELM_REINSTALL}" == true ]; then
  ...
  helm uninstall ${HELM_TLS_OPTION} -n ${CLUSTER_NAMESPACE} ${RELEASE_NAME}
fi

echo "=========================================================="
echo "DOWNLOADING STANDARD CONFIG: ${CONFIG_FILE}"
...
if [ -n "$TENANT_CONFIG_FILE" ]; then
  ...
  kubectl -n ${CLUSTER_NAMESPACE}  create secret generic ${RELEASE_NAME}-tenants \
      --from-file=tenants/${TENANT_ID}.json --dry-run -o yaml | kubectl -n ${CLUSTER_NAMESPACE} apply -f -
fi
...
if [ -n "$ADDITIONAL_APP_CONFIG_FILE" ]; then
  ...
  kubectl -n ${CLUSTER_NAMESPACE} create secret generic ${RELEASE_NAME}-config \
...
if [ -n "${DICTIONARY_CONFIG_FILE}" ]; then
  ...
  kubectl -n ${CLUSTER_NAMESPACE} create secret generic ${RELEASE_NAME}-dictionary \
      --from-file=tenants/${TENANT_ID}-dictionary.json --dry-run -o yaml | kubectl -n ${CLUSTER_NAMESPACE} apply -f -
fi
...
if [ "${HELM_DEBUG}" == true ]; then
  ...
  helm upgrade ${RELEASE_NAME} ${HELM_TLS_OPTION} -n ${CLUSTER_NAMESPACE} -f ${CONFIG_FILE} --install --debug --dry-run --set imageRepository=${IMAGE_REPOSITORY},imageName=${IMAGE_NAME},imageTag=${IMAGE_TAG},imagePullSecretName=${IMAGE_PULL_SECRET_NAME} aidt-charts/${CHART_NAME} ${HELM_UPGRADE_EXTRA_FLAGS}
fi
echo "=========================================================="
echo -e "DEPLOYING TO CLUSTER/NAMESPACE: ${CLUSTER_NAME}/${CLUSTER_NAMESPACE}."
helm upgrade ${RELEASE_NAME} ${HELM_TLS_OPTION} -n ${CLUSTER_NAMESPACE} -f ${CONFIG_FILE} --install --set imageRepository=${IMAGE_REPOSITORY},imageName=${IMAGE_NAME},imageTag=${IMAGE_TAG},imagePullSecretName=${IMAGE_PULL_SECRET_NAME} aidt-charts/${CHART_NAME} ${HELM_UPGRADE_EXTRA_FLAGS}
...
helm status ${HELM_TLS_OPTION} -n ${CLUSTER_NAMESPACE} ${RELEASE_NAME}

echo ""
echo -e "History for release: ${RELEASE_NAME}"
helm history ${HELM_TLS_OPTION} -n ${CLUSTER_NAMESPACE} ${RELEASE_NAME}
