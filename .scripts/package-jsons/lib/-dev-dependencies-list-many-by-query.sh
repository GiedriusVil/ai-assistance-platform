...
_packageJsons_devDependencies_listManyByQuery() {
  local FUNCTION_NAME="_packageJsons_devDependencies_listManyByQuery"
  ...
  for PACKAGE_JSON_PATH in $PACKAGE_JSON_PATHS; do
    ...
    then
      DEPENDENCIES=$(
        cat "${PACKAGE_JSON_PATH}" | jq ".devDependencies"
      )
    else
      local VERSION=$(
        cat "${PACKAGE_JSON_PATH}" | jq -r --arg PACKAGE_JSON_DEPENDENCY_NAME "${PACKAGE_JSON_DEPENDENCY_NAME}" '.devDependencies.[$PACKAGE_JSON_DEPENDENCY_NAME]'
      )
      DEPENDENCIES=$(
        jq -n \
