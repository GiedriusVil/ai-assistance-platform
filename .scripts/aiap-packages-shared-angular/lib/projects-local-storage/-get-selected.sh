...
_aiapPackagesSharedAngular_projectsLocalStorage_getSelected() {
  local FUNCTION_NAME="_aiapPackagesSharedAngular_projectsLocalStorage_getSelected"
  ...
  do
    _loggers_emptyLine
    _loggers_info "${FUNCTION_NAME}" "Select one of available aiap-packages-shared-angular client-projects."
    _loggers_emptyLine
    ...
    select CLIENT_PROJECT_ID in $CLIENT_PROJECT_IDS exit; do
      ...
      *)
        RET_VAL=$(
          _aiapPackagesSharedAngular_projectsLocalStorage_listOneById "${CLIENT_PROJECT_ID}"
        )
        break
        ;;
      esac
