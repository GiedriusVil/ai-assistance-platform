...
@Injectable()
export class TimezoneServiceV1 {
  ...
  userTimezone = 'America/Danmarkshavn';

  constructor(
    private sessionService: SessionServiceV1,
  ) {
    this.userTimezone = ramda.pathOr('America/Danmarkshavn', ['timezone'], this.sessionService.getUser());
  }
  ...
  overrideUserTimezone(timezone: string) {
    this.userTimezone = timezone;
  }

  getUserTimezone(user: any) {
    let retVal = this.timezones.find(timezone => timezone?.utc[0] === user?.timezone);
    if (!retVal) {
      retVal = this.timezones[0];
    }
    return retVal;
  }

  getTimeByUserTimezone(time: string) {
    return moment(time).tz(this.userTimezone).format('MM/DD/YYYY hh:mm A');
  }

  getTimeByUserTimezoneFromUnix(time: string) {
    return moment(Number(time)).tz(this.userTimezone).format('MM/DD/YY hh:mm A');
  }
}
